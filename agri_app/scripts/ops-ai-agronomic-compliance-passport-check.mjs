...
const files = {
  engine: "src/lib/ai/aiAgronomicCompliancePassport.ts",
  route: "src/app/api/ops/ai-agronomic-compliance-passport-dry-run/route.ts",
  ui: "src/app/ai/photo-diagnosis/AgronomicCompliancePassportPanel.tsx",
  admin: "src/app/admin/operations/OperationsAiAgronomicCompliancePassport.tsx",
  readme: "AI_AGRONOMIC_COMPLIANCE_PASSPORT_V11_2.md",
  pkg: "package.json",
...
requireText("engine", engine, "buildAiAgronomicCompliancePassportReport");
requireText("engine", engine, "aiAgronomicCompliancePassportVersion");
requireText("engine", engine, "AgronomicCompliancePassportReport");
requireText("engine", engine, "ComplianceSourceNode");
requireText("engine", engine, "ComplianceRequirementItem");
requireText("engine", engine, "AuditTrailEntry");
requireText("engine", engine, "CertificationReadinessItem");
requireText("engine", engine, "compliancePassportReady: true");
requireText("engine", engine, "certificationReadinessReady: true");
requireText("engine", engine, "auditPackSandboxReady: true");
requireText("engine", engine, "traceabilityReviewReady: true");
...
requireText("route", route, "/api/ops/ai-agronomic-compliance-passport-dry-run");

requireText("ui", ui, "AgronomicCompliancePassportPanel");
requireText("ui", ui, "Agronomic Compliance Passport & Certification Readiness Kernel");

requireText("admin", admin, "OperationsAiAgronomicCompliancePassport");
requireText("admin", admin, "/api/ops/ai-agronomic-compliance-passport-dry-run");

requireText("readme", readme, "AI Agronomic Compliance Passport & Certification Readiness Kernel");
requireText("package", pkg, "ops:ai-agronomic-compliance-passport-check");
requireText("runbook", runbook, "V11.2");
requireText("runbook", runbook, "ops:ai-agronomic-compliance-passport-check");
...
for (const supportScript of [
  ...
  requireText(supportScript, content, "AGRI_V11_2_AGRONOMIC_COMPLIANCE_PASSPORT_CHECK");
}

const summary = {
  ok: failures.length === 0,
  check: "ops-ai-agronomic-compliance-passport-check",
  version: "V11.2",
