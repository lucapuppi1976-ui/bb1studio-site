#!/usr/bin/env node
import fs from "node:fs";
import path from "node:path";

const appRoot = fs.existsSync(path.join(process.cwd(), "package.json"))
  ? process.cwd()
  : path.join(process.cwd(), "agri_app");

const failures = [];

function readFile(relativePath) {
  const absolutePath = path.join(appRoot, relativePath);
  if (!fs.existsSync(absolutePath)) {
    failures.push("File mancante: " + relativePath);
    return "";
  }
  return fs.readFileSync(absolutePath, "utf8");
}

function requireText(label, content, fragment) {
  if (!content.includes(fragment)) failures.push(label + ": manca " + fragment);
}

function forbidText(label, content, fragment) {
  if (content.includes(fragment)) failures.push(label + ": pattern vietato " + fragment);
}

const engine = readFile("src/lib/ops/testerSecondTesterPasswordSetupPilot.ts");
const route = readFile("src/app/api/ops/tester-second-tester-password-setup-pilot/route.ts");
const ui = readFile("src/app/ai/photo-diagnosis/TesterSecondTesterPasswordSetupPilotPanel.tsx");
const admin = readFile("src/app/admin/operations/OperationsTesterSecondTesterPasswordSetupPilot.tsx");
const readme = readFile("SECOND_TESTER_PASSWORD_SETUP_PILOT_V21_0.md");
const pkg = readFile("package.json");
const runbook = readFile("OPERATIONS_RUNBOOK_V4_14.md");

for (const fragment of [
  "buildTesterSecondTesterPasswordSetupPilotReport",
  "testerSecondTesterPasswordSetupPilotVersion",
  "SECOND_TESTER_PASSWORD_SETUP_CONFIRM",
  "CONFIRM_V21_0_SECOND_TESTER_PASSWORD_SETUP",
  "TesterSecondTesterPasswordSetupPilotReport",
  "testerSecondTesterPasswordSetupPilotReady: true",
  "protectedSecondTesterPasswordWriteRouteReady: true",
  "defaultDryRunOnly: true",
  "serverWriteEnabledRequired: true",
  "serverConfirmRequired: true",
  "bodyConfirmRequired: true",
  "oneSecondTesterPasswordPilotOnly: true",
  "passwordHashWriteOnly: true",
]) requireText("engine", engine, fragment);

for (const fragment of [
  "accountCreateAllowed: false",
  "inviteEmailSendAllowed: false",
  "publicSignupAllowed: false",
  "schemaWriteAllowed: false",
  "migrationExecutionAllowed: false",
  "providerAiReady: false",
  "providerCalled: false",
  "operationalExecutionAllowed: false",
  "humanReviewRequired: true",
  "redactedOutputOnly: true",
]) requireText("guardrail", engine, fragment);

for (const fragment of [
  "PrismaClient",
  "findUnique",
  ".update(",
  "createPasswordHash",
  "PasswordHashModule",
  "AGRI_SECOND_TESTER_PASSWORD_SETUP_ENABLED",
  "AGRI_SECOND_TESTER_PASSWORD_SETUP_CONFIRM",
  "SECOND_TESTER_PASSWORD_SETUP_CONFIRM",
  "temporaryPassword",
  "dryRun",
  "confirm",
  "writePerformed",
  "export async function GET",
  "export async function POST",
  "CRON_SECRET",
  "Accesso non consentito.",
  "/api/ops/tester-second-tester-password-setup-pilot",
]) requireText("route", route, fragment);

for (const fragment of [".create(", ".delete(", ".upsert(", ".createMany(", ".updateMany(", ".deleteMany("]) {
  forbidText("route destructive write safety", route, fragment);
}

requireText("ui", ui, "TesterSecondTesterPasswordSetupPilotPanel");
requireText("admin", admin, "OperationsTesterSecondTesterPasswordSetupPilot");
requireText("readme", readme, "Protected Second Tester Password Setup Pilot");
requireText("package", pkg, "ops:tester-second-tester-password-setup-pilot-check");
requireText("runbook", runbook, "V21.0");
requireText("runbook", runbook, "ops:tester-second-tester-password-setup-pilot-check");

for (const [label, content] of [["engine", engine], ["ui", ui], ["admin", admin]]) {
  for (const fragment of [
    ["fe", "tch("].join(""),
    ["OPENAI", "_API_KEY"].join(""),
    ["ANTHROPIC", "_API_KEY"].join(""),
    ["GEMINI", "_API_KEY"].join(""),
    ["GOOGLE", "_API_KEY"].join(""),
    ["local", "Storage"].join(""),
    ["session", "Storage"].join(""),
    ["prisma", "."].join(""),
    ["db", "."].join(""),
  ]) forbidText(label, content, fragment);
}

for (const fragment of [
  ["OPENAI", "_API_KEY"].join(""),
  ["ANTHROPIC", "_API_KEY"].join(""),
  ["GEMINI", "_API_KEY"].join(""),
  ["GOOGLE", "_API_KEY"].join(""),
  ["s", "k-"].join(""),
]) forbidText("route AI safety", route, fragment);

for (const supportScript of ["scripts/ops-runbook-check.mjs", "scripts/ops-quick-check.mjs", "scripts/ops-quick-coverage-check.mjs"]) {
  const content = readFile(supportScript);
  requireText(supportScript, content, "AGRI_V21_0_TESTER_SECOND_TESTER_PASSWORD_SETUP_PILOT_CHECK");
}

const summary = {
  ok: failures.length === 0,
  check: "ops-tester-second-tester-password-setup-pilot-check",
  version: "V21.0",
  totalFailures: failures.length,
  failures,
};

console.log(JSON.stringify(summary, null, 2));
if (failures.length > 0) process.exit(1);
