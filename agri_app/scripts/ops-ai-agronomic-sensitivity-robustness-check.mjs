...
const files = {
  engine: "src/lib/ai/aiAgronomicSensitivityRobustness.ts",
  route: "src/app/api/ops/ai-agronomic-sensitivity-robustness-dry-run/route.ts",
  ui: "src/app/ai/photo-diagnosis/AgronomicSensitivityRobustnessPanel.tsx",
  admin: "src/app/admin/operations/OperationsAiAgronomicSensitivityRobustness.tsx",
  readme: "AI_AGRONOMIC_SENSITIVITY_ROBUSTNESS_V14_3.md",
  pkg: "package.json",
...
requireText("engine", engine, "buildAiAgronomicSensitivityRobustnessReport");
requireText("engine", engine, "aiAgronomicSensitivityRobustnessVersion");
requireText("engine", engine, "AgronomicSensitivityRobustnessReport");
requireText("engine", engine, "SensitivityDriverItem");
requireText("engine", engine, "PerturbationScenarioItem");
requireText("engine", engine, "RobustnessBoundaryItem");
requireText("engine", engine, "sensitivityMapReady: true");
requireText("engine", engine, "robustnessBoundaryReady: true");
requireText("engine", engine, "nonExecutionBoundaryReady: true");
requireText("engine", engine, "humanRobustnessSignoffReady: true");
...
requireText("route", route, "/api/ops/ai-agronomic-sensitivity-robustness-dry-run");

requireText("ui", ui, "AgronomicSensitivityRobustnessPanel");
requireText("ui", ui, "Agronomic Sensitivity Map & Robustness Boundary Board");

requireText("admin", admin, "OperationsAiAgronomicSensitivityRobustness");
requireText("admin", admin, "/api/ops/ai-agronomic-sensitivity-robustness-dry-run");

requireText("readme", readme, "AI Agronomic Sensitivity Map & Robustness Boundary Board");
requireText("package", pkg, "ops:ai-agronomic-sensitivity-robustness-check");
requireText("runbook", runbook, "V14.3");
requireText("runbook", runbook, "ops:ai-agronomic-sensitivity-robustness-check");
...
for (const supportScript of [
  ...
  requireText(supportScript, content, "AGRI_V14_3_AGRONOMIC_SENSITIVITY_ROBUSTNESS_CHECK");
}

const summary = {
  ok: failures.length === 0,
  check: "ops-ai-agronomic-sensitivity-robustness-check",
  version: "V14.3",
