...
const engine = readFile("src/lib/ops/testerDualTesterUatComparativeExpansionGate.ts");
const route = readFile("src/app/api/ops/tester-dual-tester-uat-comparative-expansion-gate-dry-run/route.ts");
const ui = readFile("src/app/ai/photo-diagnosis/TesterDualTesterUatComparativeExpansionGatePanel.tsx");
const admin = readFile("src/app/admin/operations/OperationsTesterDualTesterUatComparativeExpansionGate.tsx");
const readme = readFile("DUAL_TESTER_UAT_COMPARATIVE_EXPANSION_GATE_V21_4.md");
const pkg = readFile("package.json");
const runbook = readFile("OPERATIONS_RUNBOOK_V4_14.md");

for (const fragment of [
  "buildTesterDualTesterUatComparativeExpansionReport",
  "testerDualTesterUatComparativeExpansionGateVersion",
...
requireText("ui", ui, "TesterDualTesterUatComparativeExpansionGatePanel");
requireText("admin", admin, "OperationsTesterDualTesterUatComparativeExpansionGate");
requireText("readme", readme, "Dual Tester UAT Comparative Report");
requireText("package", pkg, "ops:tester-dual-tester-uat-comparative-expansion-gate-check");
requireText("runbook", runbook, "V21.4");
requireText("runbook", runbook, "ops:tester-dual-tester-uat-comparative-expansion-gate-check");
...
for (const supportScript of ["scripts/ops-runbook-check.mjs", "scripts/ops-quick-check.mjs", "scripts/ops-quick-coverage-check.mjs"]) {
  ...
  requireText(supportScript, content, "AGRI_V21_4_TESTER_DUAL_TESTER_UAT_COMPARATIVE_EXPANSION_GATE_CHECK");
}

const summary = {
  ok: failures.length === 0,
  check: "ops-tester-dual-tester-uat-comparative-expansion-gate-check",
  version: "V21.4",
