...
const files = {
  engine: "src/lib/ai/aiProviderCanaryRollout.ts",
  route: "src/app/api/ops/ai-provider-canary-rollout-dry-run/route.ts",
  ui: "src/app/ai/photo-diagnosis/ProviderCanaryRolloutPanel.tsx",
  admin: "src/app/admin/operations/OperationsAiProviderCanaryRollout.tsx",
  readme: "AI_PROVIDER_CANARY_ROLLOUT_V12_4.md",
  pkg: "package.json",
...
requireText("engine", engine, "buildAiProviderCanaryRolloutReport");
requireText("engine", engine, "aiProviderCanaryRolloutVersion");
requireText("engine", engine, "ProviderCanaryRolloutReport");
requireText("engine", engine, "ProviderCanarySourceNode");
requireText("engine", engine, "CanaryStageCard");
requireText("engine", engine, "AcceptanceCriterion");
requireText("engine", engine, "KillSwitchDrill");
requireText("engine", engine, "providerPilotReadinessReady: true");
requireText("engine", engine, "canaryRolloutSimulatorReady: true");
requireText("engine", engine, "killSwitchRehearsalReady: true");
...
requireText("route", route, "/api/ops/ai-provider-canary-rollout-dry-run");

requireText("ui", ui, "ProviderCanaryRolloutPanel");
requireText("ui", ui, "Provider Pilot Readiness Board & Canary Rollout Simulator");

requireText("admin", admin, "OperationsAiProviderCanaryRollout");
requireText("admin", admin, "/api/ops/ai-provider-canary-rollout-dry-run");

requireText("readme", readme, "AI Provider Pilot Readiness Board & Canary Rollout Simulator");
requireText("package", pkg, "ops:ai-provider-canary-rollout-check");
requireText("runbook", runbook, "V12.4");
requireText("runbook", runbook, "ops:ai-provider-canary-rollout-check");
...
for (const supportScript of [
  ...
  requireText(supportScript, content, "AGRI_V12_4_PROVIDER_CANARY_ROLLOUT_CHECK");
}

const summary = {
  ok: failures.length === 0,
  check: "ops-ai-provider-canary-rollout-check",
  version: "V12.4",
