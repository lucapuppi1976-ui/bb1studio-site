...
const files = {
  engine: "src/lib/ai/aiProviderStagingShadowRun.ts",
  route: "src/app/api/ops/ai-provider-staging-shadow-run-dry-run/route.ts",
  ui: "src/app/ai/photo-diagnosis/ProviderStagingShadowRunPanel.tsx",
  admin: "src/app/admin/operations/OperationsAiProviderStagingShadowRun.tsx",
  readme: "AI_PROVIDER_STAGING_SHADOW_RUN_V15_4.md",
  pkg: "package.json",
...
requireText("engine", engine, "buildAiProviderStagingShadowRunReport");
requireText("engine", engine, "aiProviderStagingShadowRunVersion");
requireText("engine", engine, "ProviderStagingShadowRunReport");
requireText("engine", engine, "ShadowScenarioItem");
requireText("engine", engine, "EnvelopeSimulationItem");
requireText("engine", engine, "ContractValidationItem");
requireText("engine", engine, "providerShadowRunPlanReady: true");
requireText("engine", engine, "shadowEvaluationReady: true");
requireText("engine", engine, "fallbackDrillReady: true");
requireText("engine", engine, "nonProductionBoundaryReady: true");
requireText("engine", engine, "shadowRunExternalCallAllowed: false");
requireText("engine", engine, "shadowRunExternalCallPerformed: false");
...
requireText("route", route, "/api/ops/ai-provider-staging-shadow-run-dry-run");

requireText("ui", ui, "ProviderStagingShadowRunPanel");
requireText("ui", ui, "Provider Staging Shadow Run & Non-Production Evaluation Drill");

requireText("admin", admin, "OperationsAiProviderStagingShadowRun");
requireText("admin", admin, "/api/ops/ai-provider-staging-shadow-run-dry-run");

requireText("readme", readme, "AI Provider Staging Shadow Run & Non-Production Evaluation Drill");
requireText("package", pkg, "ops:ai-provider-staging-shadow-run-check");
requireText("runbook", runbook, "V15.4");
requireText("runbook", runbook, "ops:ai-provider-staging-shadow-run-check");
...
const trueFlags = [
  ...
  "providerShadowRunPlanReady",
  "shadowEvaluationReady",
  "fallbackDrillReady",
  "nonProductionBoundaryReady",
];
...
for (const supportScript of [
  ...
  requireText(supportScript, content, "AGRI_V15_4_PROVIDER_STAGING_SHADOW_RUN_CHECK");
}

const summary = {
  ok: failures.length === 0,
  check: "ops-ai-provider-staging-shadow-run-check",
  version: "V15.4",
