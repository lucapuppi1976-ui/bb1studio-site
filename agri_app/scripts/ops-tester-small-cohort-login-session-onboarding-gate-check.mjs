...
const engine = readFile("src/lib/ops/testerSmallCohortLoginSessionOnboardingGate.ts");
const route = readFile("src/app/api/ops/tester-small-cohort-login-session-onboarding-gate-dry-run/route.ts");
const ui = readFile("src/app/ai/photo-diagnosis/TesterSmallCohortLoginSessionOnboardingGatePanel.tsx");
const admin = readFile("src/app/admin/operations/OperationsTesterSmallCohortLoginSessionOnboardingGate.tsx");
const readme = readFile("SMALL_COHORT_LOGIN_SESSION_ONBOARDING_GATE_V21_9.md");
const pkg = readFile("package.json");
const runbook = readFile("OPERATIONS_RUNBOOK_V4_14.md");

for (const fragment of [
  "buildTesterSmallCohortLoginSessionOnboardingReport",
  "testerSmallCohortLoginSessionOnboardingGateVersion",
...
requireText("ui", ui, "TesterSmallCohortLoginSessionOnboardingGatePanel");
requireText("admin", admin, "OperationsTesterSmallCohortLoginSessionOnboardingGate");
requireText("readme", readme, "Small-Cohort Login Session");
requireText("package", pkg, "ops:tester-small-cohort-login-session-onboarding-gate-check");
requireText("runbook", runbook, "V21.9");
requireText("runbook", runbook, "ops:tester-small-cohort-login-session-onboarding-gate-check");
...
for (const supportScript of ["scripts/ops-runbook-check.mjs", "scripts/ops-quick-check.mjs", "scripts/ops-quick-coverage-check.mjs"]) {
  ...
  requireText(supportScript, content, "AGRI_V21_9_TESTER_SMALL_COHORT_LOGIN_SESSION_ONBOARDING_GATE_CHECK");
}

const summary = {
  ok: failures.length === 0,
  check: "ops-tester-small-cohort-login-session-onboarding-gate-check",
  version: "V21.9",
