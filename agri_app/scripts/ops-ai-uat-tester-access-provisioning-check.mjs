...
const engine = readFile("src/lib/ai/aiUatTesterAccessProvisioning.ts");
const route = readFile("src/app/api/ops/ai-uat-tester-access-provisioning-dry-run/route.ts");
const ui = readFile("src/app/ai/photo-diagnosis/UatTesterAccessProvisioningPanel.tsx");
const admin = readFile("src/app/admin/operations/OperationsAiUatTesterAccessProvisioning.tsx");
const readme = readFile("AI_UAT_TESTER_ACCESS_PROVISIONING_V18_5.md");
const pkg = readFile("package.json");
const runbook = readFile("OPERATIONS_RUNBOOK_V4_14.md");

for (const fragment of [
  "buildAiUatTesterAccessProvisioningReport",
  "aiUatTesterAccessProvisioningVersion",
...
requireText("ui", ui, "UatTesterAccessProvisioningPanel");
requireText("admin", admin, "OperationsAiUatTesterAccessProvisioning");
requireText("readme", readme, "UAT Tester Access Provisioning");
requireText("package", pkg, "ops:ai-uat-tester-access-provisioning-check");
requireText("runbook", runbook, "V18.5");
requireText("runbook", runbook, "ops:ai-uat-tester-access-provisioning-check");
...
for (const supportScript of [
  ...
  requireText(supportScript, content, "AGRI_V18_5_UAT_TESTER_ACCESS_PROVISIONING_CHECK");
}

const summary = {
  ok: failures.length === 0,
  check: "ops-ai-uat-tester-access-provisioning-check",
  version: "V18.5",
