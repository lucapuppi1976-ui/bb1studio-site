...
const engine = readFile("src/lib/ops/testerUatFixSprintRetestGate.ts");
const route = readFile("src/app/api/ops/tester-uat-fix-sprint-retest-gate-dry-run/route.ts");
const ui = readFile("src/app/ai/photo-diagnosis/TesterUatFixSprintRetestGatePanel.tsx");
const admin = readFile("src/app/admin/operations/OperationsTesterUatFixSprintRetestGate.tsx");
const readme = readFile("UAT_FIX_SPRINT_RETEST_GATE_V20_6.md");
const pkg = readFile("package.json");
const runbook = readFile("OPERATIONS_RUNBOOK_V4_14.md");

for (const fragment of [
  "buildTesterUatFixSprintRetestGateReport",
  "testerUatFixSprintRetestGateVersion",
...
requireText("ui", ui, "TesterUatFixSprintRetestGatePanel");
requireText("admin", admin, "OperationsTesterUatFixSprintRetestGate");
requireText("readme", readme, "UAT Fix Sprint Board");
requireText("package", pkg, "ops:tester-uat-fix-sprint-retest-gate-check");
requireText("runbook", runbook, "V20.6");
requireText("runbook", runbook, "ops:tester-uat-fix-sprint-retest-gate-check");
...
for (const supportScript of ["scripts/ops-runbook-check.mjs", "scripts/ops-quick-check.mjs", "scripts/ops-quick-coverage-check.mjs"]) {
  ...
  requireText(supportScript, content, "AGRI_V20_6_TESTER_UAT_FIX_SPRINT_RETEST_GATE_CHECK");
}

const summary = {
  ok: failures.length === 0,
  check: "ops-tester-uat-fix-sprint-retest-gate-check",
  version: "V20.6",
