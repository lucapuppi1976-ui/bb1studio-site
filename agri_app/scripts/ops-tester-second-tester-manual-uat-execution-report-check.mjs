...
const engine = readFile("src/lib/ops/testerSecondTesterManualUatExecutionReport.ts");
const route = readFile("src/app/api/ops/tester-second-tester-manual-uat-execution-report-dry-run/route.ts");
const ui = readFile("src/app/ai/photo-diagnosis/TesterSecondTesterManualUatExecutionReportPanel.tsx");
const admin = readFile("src/app/admin/operations/OperationsTesterSecondTesterManualUatExecutionReport.tsx");
const readme = readFile("SECOND_TESTER_MANUAL_UAT_EXECUTION_REPORT_V21_3.md");
const pkg = readFile("package.json");
const runbook = readFile("OPERATIONS_RUNBOOK_V4_14.md");

for (const fragment of [
  "buildTesterSecondTesterManualUatExecutionReport",
  "testerSecondTesterManualUatExecutionReportVersion",
...
requireText("ui", ui, "TesterSecondTesterManualUatExecutionReportPanel");
requireText("admin", admin, "OperationsTesterSecondTesterManualUatExecutionReport");
requireText("readme", readme, "Second Tester Manual UAT Execution Report");
requireText("package", pkg, "ops:tester-second-tester-manual-uat-execution-report-check");
requireText("runbook", runbook, "V21.3");
requireText("runbook", runbook, "ops:tester-second-tester-manual-uat-execution-report-check");
...
for (const supportScript of ["scripts/ops-runbook-check.mjs", "scripts/ops-quick-check.mjs", "scripts/ops-quick-coverage-check.mjs"]) {
  ...
  requireText(supportScript, content, "AGRI_V21_3_TESTER_SECOND_TESTER_MANUAL_UAT_EXECUTION_REPORT_CHECK");
}

const summary = {
  ok: failures.length === 0,
  check: "ops-tester-second-tester-manual-uat-execution-report-check",
  version: "V21.3",
