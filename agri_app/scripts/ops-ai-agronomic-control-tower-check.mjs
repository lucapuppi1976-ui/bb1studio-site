...
const files = {
  engine: "src/lib/ai/aiAgronomicControlTower.ts",
  route: "src/app/api/ops/ai-agronomic-control-tower-dry-run/route.ts",
  ui: "src/app/ai/photo-diagnosis/AgronomicControlTowerPanel.tsx",
  admin: "src/app/admin/operations/OperationsAiAgronomicControlTower.tsx",
  readme: "AI_AGRONOMIC_CONTROL_TOWER_V11_0.md",
  pkg: "package.json",
...
requireText("engine", engine, "buildAiAgronomicControlTowerReport");
requireText("engine", engine, "aiAgronomicControlTowerVersion");
requireText("engine", engine, "AgronomicControlTowerReport");
requireText("engine", engine, "TowerModuleNode");
requireText("engine", engine, "TowerCommandCard");
requireText("engine", engine, "TowerGovernanceGate");
requireText("engine", engine, "TowerEscalationPath");
requireText("engine", engine, "controlTowerSimulationReady: true");
requireText("engine", engine, "governanceKernelReady: true");
requireText("engine", engine, "crossModuleReviewReady: true");
requireText("engine", engine, "manualCommandCenterReady: true");
...
requireText("route", route, "/api/ops/ai-agronomic-control-tower-dry-run");

requireText("ui", ui, "AgronomicControlTowerPanel");
requireText("ui", ui, "Agronomic Control Tower & Governance Kernel");

requireText("admin", admin, "OperationsAiAgronomicControlTower");
requireText("admin", admin, "/api/ops/ai-agronomic-control-tower-dry-run");

requireText("readme", readme, "AI Agronomic Control Tower & Governance Kernel");
requireText("package", pkg, "ops:ai-agronomic-control-tower-check");
requireText("runbook", runbook, "V11.0");
requireText("runbook", runbook, "ops:ai-agronomic-control-tower-check");
...
for (const supportScript of [
  ...
  requireText(supportScript, content, "AGRI_V11_0_AGRONOMIC_CONTROL_TOWER_CHECK");
}

const summary = {
  ok: failures.length === 0,
  check: "ops-ai-agronomic-control-tower-check",
  version: "V11.0",
