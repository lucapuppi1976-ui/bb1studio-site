...
const files = {
  engine: "src/lib/ai/aiMigrationRehearsalStorageGate.ts",
  route: "src/app/api/ops/ai-migration-rehearsal-storage-gate-dry-run/route.ts",
  ui: "src/app/ai/photo-diagnosis/MigrationRehearsalStorageGatePanel.tsx",
  admin: "src/app/admin/operations/OperationsAiMigrationRehearsalStorageGate.tsx",
  readme: "AI_MIGRATION_REHEARSAL_STORAGE_GATE_V15_5.md",
  pkg: "package.json",
...
requireText("engine", engine, "buildAiMigrationRehearsalStorageGateReport");
requireText("engine", engine, "aiMigrationRehearsalStorageGateVersion");
requireText("engine", engine, "MigrationRehearsalStorageGateReport");
requireText("engine", engine, "RehearsalPlanItem");
requireText("engine", engine, "SchemaDiffReviewItem");
requireText("engine", engine, "BackupRestoreDrillItem");
requireText("engine", engine, "stagingRehearsalPlanReady: true");
requireText("engine", engine, "storageGateValidationReady: true");
requireText("engine", engine, "rollbackDrillPlanReady: true");
requireText("engine", engine, "retentionValidationReady: true");
requireText("engine", engine, "migrationExecutionAllowed: false");
requireText("engine", engine, "schemaWriteAllowed: false");
requireText("engine", engine, "schemaWritePerformed: false");
...
requireText("route", route, "/api/ops/ai-migration-rehearsal-storage-gate-dry-run");

requireText("ui", ui, "MigrationRehearsalStorageGatePanel");
requireText("ui", ui, "Migration Rehearsal & Storage Gate Validation in Staging");

requireText("admin", admin, "OperationsAiMigrationRehearsalStorageGate");
requireText("admin", admin, "/api/ops/ai-migration-rehearsal-storage-gate-dry-run");

requireText("readme", readme, "AI Migration Rehearsal & Storage Gate Validation in Staging");
requireText("package", pkg, "ops:ai-migration-rehearsal-storage-gate-check");
requireText("runbook", runbook, "V15.5");
requireText("runbook", runbook, "ops:ai-migration-rehearsal-storage-gate-check");
...
const trueFlags = [
  ...
  "stagingRehearsalPlanReady",
  "storageGateValidationReady",
  "rollbackDrillPlanReady",
  "retentionValidationReady",
];
...
for (const supportScript of [
  ...
  requireText(supportScript, content, "AGRI_V15_5_MIGRATION_REHEARSAL_STORAGE_GATE_CHECK");
}

const summary = {
  ok: failures.length === 0,
  check: "ops-ai-migration-rehearsal-storage-gate-check",
  version: "V15.5",
