...
const engine = readFile("src/lib/ops/testerSecondTesterPoolExpansionGate.ts");
const route = readFile("src/app/api/ops/tester-second-tester-pool-expansion-gate-dry-run/route.ts");
const ui = readFile("src/app/ai/photo-diagnosis/TesterSecondTesterPoolExpansionGatePanel.tsx");
const admin = readFile("src/app/admin/operations/OperationsTesterSecondTesterPoolExpansionGate.tsx");
const readme = readFile("SECOND_TESTER_POOL_EXPANSION_GATE_V20_7.md");
const pkg = readFile("package.json");
const runbook = readFile("OPERATIONS_RUNBOOK_V4_14.md");

for (const fragment of [
  "buildTesterSecondTesterPoolExpansionReport",
  "testerSecondTesterPoolExpansionGateVersion",
...
requireText("ui", ui, "TesterSecondTesterPoolExpansionGatePanel");
requireText("admin", admin, "OperationsTesterSecondTesterPoolExpansionGate");
requireText("readme", readme, "Controlled Second Tester Pool Expansion Gate");
requireText("package", pkg, "ops:tester-second-tester-pool-expansion-gate-check");
requireText("runbook", runbook, "V20.7");
requireText("runbook", runbook, "ops:tester-second-tester-pool-expansion-gate-check");
...
for (const supportScript of ["scripts/ops-runbook-check.mjs", "scripts/ops-quick-check.mjs", "scripts/ops-quick-coverage-check.mjs"]) {
  ...
  requireText(supportScript, content, "AGRI_V20_7_TESTER_SECOND_TESTER_POOL_EXPANSION_GATE_CHECK");
}

const summary = {
  ok: failures.length === 0,
  check: "ops-tester-second-tester-pool-expansion-gate-check",
  version: "V20.7",
