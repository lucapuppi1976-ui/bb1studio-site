...
const files = {
  engine: "src/lib/ai/aiSoilNutrientStrategy.ts",
  route: "src/app/api/ops/ai-soil-nutrient-strategy-dry-run/route.ts",
  ui: "src/app/ai/photo-diagnosis/SoilNutrientStrategyPanel.tsx",
  admin: "src/app/admin/operations/OperationsAiSoilNutrientStrategy.tsx",
  readme: "AI_SOIL_NUTRIENT_STRATEGY_V10_7.md",
  pkg: "package.json",
...
requireText("engine", engine, "buildAiSoilNutrientStrategyReport");
requireText("engine", engine, "aiSoilNutrientStrategyVersion");
requireText("engine", engine, "SoilNutrientStrategyReport");
requireText("engine", engine, "SoilZone");
requireText("engine", engine, "NutrientBalanceSignal");
requireText("engine", engine, "SoilStrategyScenario");
requireText("engine", engine, "SoilGovernanceStop");
requireText("engine", engine, "soilHealthSimulationReady: true");
requireText("engine", engine, "nutrientBalanceSandboxReady: true");
requireText("engine", engine, "samplingPlanPreviewReady: true");
requireText("engine", engine, "soilEvidenceReviewReady: true");
...
requireText("route", route, "/api/ops/ai-soil-nutrient-strategy-dry-run");

requireText("ui", ui, "SoilNutrientStrategyPanel");
requireText("ui", ui, "Soil Health & Nutrient Balance Strategy Simulator");

requireText("admin", admin, "OperationsAiSoilNutrientStrategy");
requireText("admin", admin, "/api/ops/ai-soil-nutrient-strategy-dry-run");

requireText("readme", readme, "AI Soil Health & Nutrient Balance Strategy Simulator");
requireText("package", pkg, "ops:ai-soil-nutrient-strategy-check");
requireText("runbook", runbook, "V10.7");
requireText("runbook", runbook, "ops:ai-soil-nutrient-strategy-check");
...
for (const supportScript of [
  ...
  requireText(supportScript, content, "AGRI_V10_7_SOIL_NUTRIENT_STRATEGY_CHECK");
}

const summary = {
  ok: failures.length === 0,
  check: "ops-ai-soil-nutrient-strategy-check",
  version: "V10.7",
