...
const files = {
  engine: "src/lib/ai/aiAgronomicExplainabilityLedger.ts",
  route: "src/app/api/ops/ai-agronomic-explainability-ledger-dry-run/route.ts",
  ui: "src/app/ai/photo-diagnosis/AgronomicExplainabilityLedgerPanel.tsx",
  admin: "src/app/admin/operations/OperationsAiAgronomicExplainabilityLedger.tsx",
  readme: "AI_AGRONOMIC_EXPLAINABILITY_LEDGER_V11_1.md",
  pkg: "package.json",
...
requireText("engine", engine, "buildAiAgronomicExplainabilityLedgerReport");
requireText("engine", engine, "aiAgronomicExplainabilityLedgerVersion");
requireText("engine", engine, "AgronomicExplainabilityLedgerReport");
requireText("engine", engine, "ExplanationSourceNode");
requireText("engine", engine, "EvidenceContributionItem");
requireText("engine", engine, "UncertaintyRegisterItem");
requireText("engine", engine, "LedgerAuditItem");
requireText("engine", engine, "explainabilityLedgerReady: true");
requireText("engine", engine, "traceabilityKernelReady: true");
requireText("engine", engine, "uncertaintyRegisterReady: true");
requireText("engine", engine, "reviewerQuestionBuilderReady: true");
...
requireText("route", route, "/api/ops/ai-agronomic-explainability-ledger-dry-run");

requireText("ui", ui, "AgronomicExplainabilityLedgerPanel");
requireText("ui", ui, "Agronomic Explainability Ledger & Traceability Kernel");

requireText("admin", admin, "OperationsAiAgronomicExplainabilityLedger");
requireText("admin", admin, "/api/ops/ai-agronomic-explainability-ledger-dry-run");

requireText("readme", readme, "AI Agronomic Explainability Ledger & Traceability Kernel");
requireText("package", pkg, "ops:ai-agronomic-explainability-ledger-check");
requireText("runbook", runbook, "V11.1");
requireText("runbook", runbook, "ops:ai-agronomic-explainability-ledger-check");
...
for (const supportScript of [
  ...
  requireText(supportScript, content, "AGRI_V11_1_AGRONOMIC_EXPLAINABILITY_LEDGER_CHECK");
}

const summary = {
  ok: failures.length === 0,
  check: "ops-ai-agronomic-explainability-ledger-check",
  version: "V11.1",
