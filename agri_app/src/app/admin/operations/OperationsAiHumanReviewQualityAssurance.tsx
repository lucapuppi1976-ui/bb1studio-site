...
export default function OperationsAiHumanReviewQualityAssurance() {
  const report = buildAiHumanReviewQualityAssuranceReport({
    qaCaseCount: 20,
    peerReviewGapCount: 5,
    rubricDeviationCount: 5,
    reviewerDriftSignalCount: 4,
    qaExceptionCount: 4,
    ...
    boardPackScore: 72,
    rubricQualityScore: 71,
    reviewerCalibrationScore: 72,
    qaLeadRole: "operations human review QA lead",
  });
    ...
    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <h2 className="mt-2 text-xl font-bold text-slate-950">
            AI Human Review Quality Assurance
          </h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
            Pannello operativo per peer calibration, rubric quality, reviewer drift watch,
            QA gates, exception register, audit replay e human QA signoff.
          </p>
        </div>
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-5">
        <div className="rounded-2xl bg-slate-50 p-4">
          ...
          <p className="mt-2 text-xl font-bold text-slate-950">{report.qaStatus}</p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
          <p className="text-xs uppercase tracking-wide text-slate-500">Score</p>
          <p className="mt-2 text-2xl font-bold text-slate-950">{report.qaScore}</p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
        ...
        <div className="rounded-2xl bg-slate-50 p-4">
          ...
          <p className="mt-2 text-2xl font-bold text-slate-950">{report.qaRiskRegister.length}</p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-2">
        <div className="rounded-2xl border border-slate-200 p-4">
          ...
          <div className="mt-3 space-y-3">
            {report.qaExceptionRegister.map((item) => (
              <div key={item.id} className="rounded-xl bg-slate-50 p-4">
                <p className="text-sm font-semibold text-slate-950">{item.label}</p>
                <p className="mt-1 text-xs text-slate-500">
                  {item.priority} · {item.severity} · {item.blocksQaBoard ? "blocks" : "review"}
                </p>
