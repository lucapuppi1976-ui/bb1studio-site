...
export default function OperationsAiAgronomicBoardPack() {
  const report = buildAiAgronomicBoardPackReport({
    cropPortfolio: ["tomato", "vineyard", "citrus", "olive"],
    activeCaseCount: 7,
    highRiskFieldCount: 3,
    unresolvedEvidenceGaps: 4,
    interventionReadinessScore: 68,
    memoryQualityScore: 82,
    knowledgeGovernanceScore: 76,
    autopilotSimulationScore: 73,
    protocolCoverageScore: 79,
    roiSensitivity: "balanced",
    reviewerRole: "executive agronomic reviewer",
  });
    ...
    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <h2 className="mt-2 text-xl font-bold text-slate-950">
            AI Agronomic Board Pack & Executive Decision Center
          </h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
            Pannello operativo per controllare board status, decision cards, ROI proxy,
            signal sources, governance checklist e hard-stop safety del board pack.
          </p>
        </div>
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-5">
        <div className="rounded-2xl bg-slate-50 p-4">
          ...
          <p className="mt-2 text-xl font-bold text-slate-950">{report.boardStatus}</p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
          <p className="text-xs uppercase tracking-wide text-slate-500">Score</p>
          <p className="mt-2 text-2xl font-bold text-slate-950">{report.boardScore}</p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
          <p className="text-xs uppercase tracking-wide text-slate-500">Cards</p>
          <p className="mt-2 text-2xl font-bold text-slate-950">{report.decisionCards.length}</p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-2">
        <div className="rounded-2xl border border-slate-200 p-4">
          ...
          <div className="mt-3 space-y-3">
            {report.riskRegister.map((item) => (
              <div key={item.id} className="rounded-xl bg-slate-50 p-4">
                <p className="text-sm font-semibold text-slate-950">{item.riskStatement}</p>
                <p className="mt-1 text-xs text-slate-500">
                  {item.riskTier} · exposure proxy {item.exposureProxy}
                </p>
