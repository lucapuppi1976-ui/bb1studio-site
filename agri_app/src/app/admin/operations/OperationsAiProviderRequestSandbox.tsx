...
export default function OperationsAiProviderRequestSandbox() {
  const report = buildAiProviderRequestSandboxReport({
    activeCaseCount: 9,
    pendingRequestReviewCount: 5,
    unresolvedPayloadItemCount: 5,
    unresolvedRedactionItemCount: 6,
    ...
    redactionContractScore: 74,
    outputSchemaScore: 71,
    reviewerConfidenceScore: 74,
    reviewerRole: "operations provider request reviewer",
  });
    ...
    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <h2 className="mt-2 text-xl font-bold text-slate-950">
            AI Provider Request Sandbox
          </h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
            Pannello operativo per controllare request blueprint, redaction contract,
            output contract, review gates, rollback plan e manual approval board senza chiamare provider.
          </p>
        </div>
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-5">
        <div className="rounded-2xl bg-slate-50 p-4">
          ...
          <p className="mt-2 text-xl font-bold text-slate-950">{report.requestSandboxStatus}</p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
          <p className="text-xs uppercase tracking-wide text-slate-500">Score</p>
          <p className="mt-2 text-2xl font-bold text-slate-950">{report.requestSandboxScore}</p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
        ...
        <div className="rounded-2xl bg-slate-50 p-4">
          ...
          <p className="mt-2 text-2xl font-bold text-slate-950">{report.requestGaps.length}</p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
