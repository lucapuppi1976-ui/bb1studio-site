...
export default function OperationsAiEvidenceIntegrityCustody() {
  const report = buildAiEvidenceIntegrityCustodyReport({
    evidenceItemCount: 18,
    unresolvedCustodyGapCount: 5,
    tamperReviewFlagCount: 4,
    redactionConcernCount: 4,
    replayAuditItemCount: 8,
    evidenceQualityScore: 74,
    custodyCompletenessScore: 72,
    chainContinuityScore: 71,
    replayAuditScore: 73,
    reviewerConfidenceScore: 74,
    consensusCalibrationScore: 72,
    missionControlScore: 73,
    leadReviewerRole: "operations evidence custody lead reviewer",
  });
    ...
    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <h2 className="mt-2 text-xl font-bold text-slate-950">
            AI Evidence Integrity Custody Board
          </h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
            Pannello operativo per chain-of-custody, integrity gates, tamper review,
            redaction ledger, audit replay, evidence locks e signoff umano.
          </p>
        </div>
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-5">
        <div className="rounded-2xl bg-slate-50 p-4">
          ...
          <p className="mt-2 text-xl font-bold text-slate-950">{report.integrityStatus}</p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
          <p className="text-xs uppercase tracking-wide text-slate-500">Score</p>
          <p className="mt-2 text-2xl font-bold text-slate-950">{report.integrityScore}</p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
        ...
        <div className="rounded-2xl bg-slate-50 p-4">
          ...
          <p className="mt-2 text-2xl font-bold text-slate-950">{report.integrityRiskRegister.length}</p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-2">
        <div className="rounded-2xl border border-slate-200 p-4">
          ...
          <div className="mt-3 space-y-3">
            {report.tamperReviewBoard.map((item) => (
              <div key={item.id} className="rounded-xl bg-slate-50 p-4">
                <p className="text-sm font-semibold text-slate-950">{item.label}</p>
                <p className="mt-1 text-xs text-slate-500">
                  {item.severity} · {item.flagScore}/100 · {item.blocksCustodySignoff ? "blocks" : "review"}
                </p>
