...
export default function OperationsAiAgronomicExecutiveDossier() {
  const report = buildAiAgronomicExecutiveDossierReport({
    executiveSectionCount: 14,
    openBoardQuestionCount: 5,
    unresolvedNarrativeGapCount: 5,
    boundaryCaveatCount: 5,
    signoffAgendaItemCount: 6,
    decisionSimulationScore: 72,
    scenarioStressScore: 71,
    portfolioComparatorScore: 71,
    sensitivityRobustnessScore: 71,
    governanceFreezeScore: 71,
    evidenceIntegrityScore: 73,
    qualityAssuranceScore: 72,
    complianceAttestationScore: 72,
    dossierRubricScore: 71,
    reviewerConfidenceScore: 74,
    executiveLeadRole: "operations agronomic executive board lead",
  });
    ...
    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <h2 className="mt-2 text-xl font-bold text-slate-950">
            AI Agronomic Executive Board Dossier
          </h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
            Pannello operativo per executive dossier, strategy narrative freeze,
            board signal summary, decision boundary summary, human signoff agenda e non-operational certificate.
          </p>
        </div>
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-5">
        <div className="rounded-2xl bg-slate-50 p-4">
          ...
          <p className="mt-2 text-xl font-bold text-slate-950">{report.executiveDossierStatus}</p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
          <p className="text-xs uppercase tracking-wide text-slate-500">Score</p>
          <p className="mt-2 text-2xl font-bold text-slate-950">{report.executiveDossierScore}</p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
        ...
        <div className="rounded-2xl bg-slate-50 p-4">
          ...
          <p className="mt-2 text-2xl font-bold text-slate-950">{report.executiveRiskRegister.length}</p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-2">
        <div className="rounded-2xl border border-slate-200 p-4">
          ...
          <div className="mt-3 space-y-3">
            {report.executiveBoardPack.map((item) => (
