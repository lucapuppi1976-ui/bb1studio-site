...
export default function OperationsAiPhenologyYieldRisk() {
  const report = buildAiPhenologyYieldRiskReport({
    cropPortfolio: ["tomato", "vineyard", "citrus", "olive"],
    fieldCount: 8,
    activeCaseCount: 6,
    floweringWindowCompletenessScore: 67,
    pollinationEvidenceScore: 61,
    fruitSetEvidenceScore: 64,
    heatStressRiskScore: 74,
    humidityRiskScore: 69,
    waterStressRiskScore: 76,
    pestDiseasePressureScore: 71,
    soilNutrientStrategyScore: 74,
    climateWaterStrategyScore: 76,
    digitalTwinScore: 75,
    boardPackScore: 77,
    reviewerRole: "operations phenology reviewer",
  });
    ...
    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <h2 className="mt-2 text-xl font-bold text-slate-950">
            AI Phenology, Pollination & Yield Risk Simulator
          </h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
            Pannello operativo per controllare finestre fenologiche, pollination risk,
            fruit-set evidence, yield-risk proxy, evidence gaps e hard-stop safety.
          </p>
        </div>
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-5">
        ...
        <div className="rounded-2xl bg-slate-50 p-4">
          ...
          <p className="mt-2 text-xl font-bold text-slate-950">{report.farmPhenologyRiskTier}</p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
          <p className="text-xs uppercase tracking-wide text-slate-500">Signals</p>
          <p className="mt-2 text-2xl font-bold text-slate-950">{report.riskSignals.length}</p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
