...
export default function OperationsAiTesterAccountWritePathFinalApproval() {
    ...
    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <h2 className="mt-2 text-xl font-bold text-slate-950">
            Tester Account Write Path Final Approval
          </h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
            Gate operativo dry-run per approvazione finale, account write boundary,
            rollback plan, manual creation checklist e audit packet.
          </p>
        </div>
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-5">
        <div className="rounded-2xl bg-slate-50 p-4">
          ...
          <p className="mt-2 text-xl font-bold text-slate-950">{report.finalWritePathStatus}</p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
          <p className="text-xs uppercase tracking-wide text-slate-500">Score</p>
          <p className="mt-2 text-2xl font-bold text-slate-950">{report.finalWritePathScore}</p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
      ...
      <div className="mt-5 rounded-2xl border border-slate-200 p-4">
        ...
        <ul className="mt-3 space-y-2 text-sm text-slate-700">
          ...
          <li>Rollback execution allowed: {report.readiness.rollbackExecutionAllowed ? "yes" : "no"}</li>
