import { buildAiAuthUserSchemaReadinessReport } from "@/lib/ai/aiAuthUserSchemaReadiness";

export default function OperationsAiAuthUserSchemaReadiness() {
  const report = buildAiAuthUserSchemaReadinessReport();
    ...
    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <h2 className="mt-2 text-xl font-bold text-slate-950">
            Auth/User Schema Readiness
          </h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
            Audit operativo dry-run per auth, user schema, role/language fields,
            invite plan, revocation plan e no-go board.
          </p>
        </div>
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-5">
        <div className="rounded-2xl bg-slate-50 p-4">
          ...
          <p className="mt-2 text-xl font-bold text-slate-950">{report.accountCreationPlanStatus}</p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
          <p className="text-xs uppercase tracking-wide text-slate-500">Score</p>
          <p className="mt-2 text-2xl font-bold text-slate-950">{report.accountCreationPlanScore}</p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
