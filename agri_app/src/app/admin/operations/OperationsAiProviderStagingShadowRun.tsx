...
export default function OperationsAiProviderStagingShadowRun() {
  const report = buildAiProviderStagingShadowRunReport({
    shadowScenarioCount: 6,
    envelopeSimulationCount: 7,
    contractValidationCount: 7,
    budgetSimulationCount: 6,
    fallbackDrillCount: 6,
    evaluationMatrixItemCount: 7,
    openShadowGapCount: 8,
    onlineReadinessScore: 80,
    providerGatewayScore: 70,
    storageBlueprintScore: 66,
    humanReviewWorkflowScore: 68,
    contractGuardScore: 72,
    budgetGuardScore: 64,
    fallbackDrillScore: 66,
    shadowLeadRole: "operations provider staging shadow reviewer",
  });
    ...
    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <h2 className="mt-2 text-xl font-bold text-slate-950">
            AI Provider Staging Shadow Run
          </h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
            Pannello operativo per shadow scenarios, envelope simulation, contract validation,
            budget simulation, fallback drill, evaluation matrix e non-production boundary.
          </p>
        </div>
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-5">
        <div className="rounded-2xl bg-slate-50 p-4">
          ...
          <p className="mt-2 text-xl font-bold text-slate-950">{report.shadowRunStatus}</p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
          <p className="text-xs uppercase tracking-wide text-slate-500">Score</p>
          <p className="mt-2 text-2xl font-bold text-slate-950">{report.shadowRunScore}</p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
        ...
        <div className="rounded-2xl bg-slate-50 p-4">
          ...
          <p className="mt-2 text-xl font-bold text-slate-950">
            {report.readiness.shadowRunExternalCallPerformed ? "YES" : "NO"}
          </p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
          <p className="text-xs uppercase tracking-wide text-slate-500">Risks</p>
          <p className="mt-2 text-2xl font-bold text-slate-950">{report.shadowRunRiskRegister.length}</p>
        </div>
      </div>
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-2">
        <div className="rounded-2xl border border-slate-200 p-4">
          ...
          <div className="mt-3 space-y-3">
            {report.shadowRunBoardPack.map((item) => (
        ...
        <div className="rounded-2xl border border-slate-200 p-4">
          ...
          <ul className="mt-3 space-y-2 text-sm text-slate-700">
            ...
            <li>Shadow external call allowed: {report.readiness.shadowRunExternalCallAllowed ? "yes" : "no"}</li>
            <li>Shadow external call performed: {report.readiness.shadowRunExternalCallPerformed ? "yes" : "no"}</li>
