...
export default function OperationsAiMigrationRehearsalStorageGate() {
  const report = buildAiMigrationRehearsalStorageGateReport({
    rehearsalStepCount: 8,
    schemaReviewItemCount: 7,
    backupRestoreItemCount: 7,
    storageGateItemCount: 8,
    retentionValidationItemCount: 6,
    auditEventItemCount: 6,
    openRehearsalGapCount: 8,
    onlineReadinessScore: 80,
    providerShadowScore: 70,
    storageBlueprintScore: 68,
    humanReviewWorkflowScore: 70,
    backupRestoreScore: 60,
    retentionControlScore: 62,
    rollbackReadinessScore: 64,
    rehearsalLeadRole: "operations migration rehearsal storage reviewer",
  });
    ...
    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <h2 className="mt-2 text-xl font-bold text-slate-950">
            AI Migration Rehearsal Storage Gate
          </h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
            Pannello operativo per migration rehearsal, schema diff review, backup/restore drill,
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-5">
        <div className="rounded-2xl bg-slate-50 p-4">
          ...
          <p className="mt-2 text-xl font-bold text-slate-950">{report.rehearsalStatus}</p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
          <p className="text-xs uppercase tracking-wide text-slate-500">Score</p>
          <p className="mt-2 text-2xl font-bold text-slate-950">{report.rehearsalScore}</p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
        ...
        <div className="rounded-2xl bg-slate-50 p-4">
          ...
          <p className="mt-2 text-2xl font-bold text-slate-950">{report.rehearsalRiskRegister.length}</p>
        </div>
      </div>
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-2">
        <div className="rounded-2xl border border-slate-200 p-4">
          ...
          <div className="mt-3 space-y-3">
            {report.rehearsalBoardPack.map((item) => (
