...
export default function OperationsAiCropProtocolBuilder() {
  const report = buildAiCropProtocolBuilderReport({
    cropFamily: "tomato",
    growthStage: "fruit set",
    symptomCluster: "canopy stress leaf discoloration",
    climateContext: "heat stress and irrigation variability",
    ...
    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <h2 className="mt-2 text-xl font-bold text-slate-950">
            AI Crop-Specific Expert Protocol Builder
          </h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
            Pannello operativo per verificare protocolli expert specifici per coltura,
            evidence gates, conflitti, review board e hard-stop dry-run.
          </p>
        </div>
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-5">
        <div className="rounded-2xl bg-slate-50 p-4">
          ...
          <p className="mt-2 text-xl font-bold text-slate-950">{report.protocolStatus}</p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
          <p className="text-xs uppercase tracking-wide text-slate-500">Score</p>
          <p className="mt-2 text-2xl font-bold text-slate-950">{report.protocolScore}</p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
          <p className="text-xs uppercase tracking-wide text-slate-500">Protocols</p>
          <p className="mt-2 text-2xl font-bold text-slate-950">{report.protocolVariants.length}</p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
          <p className="text-xs uppercase tracking-wide text-slate-500">Conflicts</p>
          <p className="mt-2 text-2xl font-bold text-slate-950">{report.conflicts.length}</p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-2">
        <div className="rounded-2xl border border-slate-200 p-4">
          ...
          <div className="mt-3 space-y-3">
            {report.evidenceBacklog.map((gate) => (
              <div key={gate.id} className="rounded-xl bg-slate-50 p-4">
                <p className="text-sm font-semibold text-slate-950">{gate.label}</p>
                <p className="mt-1 text-xs text-slate-500">
                  {gate.currentLevel} → {gate.requiredLevel} · blocking: {gate.blocking ? "yes" : "no"}
                </p>
                <p className="mt-2 text-xs text-slate-600">{gate.manualCollectionAction}</p>
              </div>
            ))}
          </div>
        </div>
