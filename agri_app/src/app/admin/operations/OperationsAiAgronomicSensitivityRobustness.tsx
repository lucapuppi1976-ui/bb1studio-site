...
export default function OperationsAiAgronomicSensitivityRobustness() {
  const report = buildAiAgronomicSensitivityRobustnessReport({
    sensitivityDriverCount: 6,
    perturbationScenarioCount: 7,
    fragileAssumptionCount: 6,
    uncertaintyLeverCount: 6,
    unresolvedRobustnessQuestionCount: 5,
    ...
    robustnessRubricScore: 71,
    reviewerConfidenceScore: 74,
    robustnessLeadRole: "operations agronomic robustness board lead",
  });
    ...
    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <h2 className="mt-2 text-xl font-bold text-slate-950">
            AI Agronomic Sensitivity Robustness Board
          </h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
            Pannello operativo per sensitivity drivers, perturbation matrix,
            robustness boundaries, fragile assumptions, uncertainty levers, non-execution boundary e signoff umano.
          </p>
        </div>
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-5">
        <div className="rounded-2xl bg-slate-50 p-4">
          ...
          <p className="mt-2 text-xl font-bold text-slate-950">{report.robustnessStatus}</p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
          <p className="text-xs uppercase tracking-wide text-slate-500">Score</p>
          <p className="mt-2 text-2xl font-bold text-slate-950">{report.robustnessScore}</p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
        ...
        <div className="rounded-2xl bg-slate-50 p-4">
          ...
          <p className="mt-2 text-2xl font-bold text-slate-950">{report.robustnessRiskRegister.length}</p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-2">
        <div className="rounded-2xl border border-slate-200 p-4">
          ...
          <div className="mt-3 space-y-3">
            {report.robustnessBoardPack.map((item) => (
