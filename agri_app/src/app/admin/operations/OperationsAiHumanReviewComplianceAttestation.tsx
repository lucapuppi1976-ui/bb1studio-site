...
export default function OperationsAiHumanReviewComplianceAttestation() {
  const report = buildAiHumanReviewComplianceAttestationReport({
    attestationItemCount: 18,
    unresolvedComplianceQuestionCount: 5,
    accountabilityGapCount: 5,
    governanceExceptionCount: 4,
    signoffVarianceCount: 4,
    ...
    complianceRubricScore: 71,
    accountabilityConfidenceScore: 72,
    complianceLeadRole: "operations human review compliance lead",
  });
    ...
    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <h2 className="mt-2 text-xl font-bold text-slate-950">
            AI Human Review Compliance Attestation
          </h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
            Pannello operativo per compliance attestation, accountability ledger,
            governance evidence pack, compliance gates, exception register, audit replay e human signoff.
          </p>
        </div>
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-5">
        <div className="rounded-2xl bg-slate-50 p-4">
          ...
          <p className="mt-2 text-xl font-bold text-slate-950">{report.attestationStatus}</p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
          <p className="text-xs uppercase tracking-wide text-slate-500">Score</p>
          <p className="mt-2 text-2xl font-bold text-slate-950">{report.attestationScore}</p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
        ...
        <div className="rounded-2xl bg-slate-50 p-4">
          ...
          <p className="mt-2 text-2xl font-bold text-slate-950">{report.complianceRiskRegister.length}</p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-2">
        <div className="rounded-2xl border border-slate-200 p-4">
          ...
          <div className="mt-3 space-y-3">
            {report.governanceEvidencePack.map((item) => (
