...
export default function OperationsAiAgronomicImprovementScorecard() {
  const report = buildAiAgronomicImprovementScorecardReport({
    activeCaseCount: 9,
    openImprovementItemCount: 7,
    ...
    scenarioStressScore: 67,
    evidenceQualityScore: 69,
    governanceScore: 73,
    fieldOpsScore: 71,
    qualityScore: 72,
    reviewerConfidenceScore: 73,
    reviewerRole: "operations continuous improvement reviewer",
  });
    ...
    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <h2 className="mt-2 text-xl font-bold text-slate-950">
            AI Agronomic Continuous Improvement Scorecard
          </h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
            Pannello operativo per controllare scorecard resilienza, backlog, maturity model,
            review cadence, roadmap scenarios, improvement gaps e hard-stop safety.
          </p>
        </div>
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-5">
        <div className="rounded-2xl bg-slate-50 p-4">
          ...
          <p className="mt-2 text-xl font-bold text-slate-950">{report.improvementStatus}</p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
          <p className="text-xs uppercase tracking-wide text-slate-500">Score</p>
          <p className="mt-2 text-2xl font-bold text-slate-950">{report.improvementScore}</p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
        ...
        <div className="rounded-2xl bg-slate-50 p-4">
          ...
          <p className="mt-2 text-2xl font-bold text-slate-950">{report.improvementGaps.length}</p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-2">
        <div className="rounded-2xl border border-slate-200 p-4">
          ...
          <div className="mt-3 space-y-3">
            {report.maturityModel.map((item) => (
              <div key={item.id} className="rounded-xl bg-slate-50 p-4">
                <p className="text-sm font-semibold text-slate-950">{item.label}</p>
                <p className="mt-1 text-xs text-slate-500">
                  {item.lane} · level {item.currentLevel} → {item.targetLevel}
                </p>
