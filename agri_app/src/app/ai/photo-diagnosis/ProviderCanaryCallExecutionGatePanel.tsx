...
export default function ProviderCanaryCallExecutionGatePanel() {
  const report = useMemo(() => buildAiProviderCanaryCallExecutionGateReport(), []);
  const failedGates = report.providerCallGateChecks.filter((gate) => !gate.passed);
  const blockingFindings = report.providerCallFindings.filter((item) => item.blocksProviderCallActivation);
  const noGoItems = report.providerCallNoGoBoard.filter((item) => item.goNoGoState === "no-go");
    ...
    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.22em] text-emerald-700">
            V17.2 · AI Provider Canary Call Execution Gate
          </p>
          <h2 className="mt-2 text-2xl font-bold text-slate-950">
            Provider Canary Call Execution Gate & Explicit Provider Approval Lock
          </h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
            Gate locale dry-run per futura chiamata provider canary, explicit provider approval,
            request boundary, budget envelope, result boundary e rollback call plan.
            Nessuna chiamata provider viene eseguita.
          </p>
        </div>

        <div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-900">
          <p className="font-semibold">Provider call: {report.providerCallStatus}</p>
          <p className="mt-1">Score: {report.providerCallScore}/100 · severity: {report.overallSeverity}</p>
        </div>
      </div>
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-5">
        <div className="rounded-2xl bg-slate-50 p-4">
          ...
          <p className="mt-2 text-2xl font-bold text-slate-950">
            {report.readiness.providerCanaryCallExecutionGateReady ? "READY" : "NO"}
          </p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
          <p className="text-xs uppercase tracking-wide text-slate-500">Provider call</p>
          <p className="mt-2 text-2xl font-bold text-slate-950">
            {report.readiness.providerCanaryCallExecutionAllowed ? "YES" : "LOCKED"}
          </p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-2">
        <div className="rounded-2xl border border-slate-200 p-4">
          ...
          <div className="mt-4 space-y-3">
            {report.providerCallGate.map((item) => (
          ...
          <div className="mt-4 space-y-3">
            {report.providerRequestBoundary.map((item) => (
              <div key={item.id} className="rounded-xl bg-slate-50 p-4">
                <p className="text-sm font-semibold text-slate-950">{item.label}</p>
                <p className="mt-1 text-xs text-slate-500">
        ...
        <div className="rounded-2xl border border-slate-200 p-4">
          ...
          <div className="mt-4 space-y-3">
            {report.providerCallGateChecks.map((gate) => (
          ...
          <div className="mt-4 space-y-3">
            {report.providerCallNoGoBoard.map((item) => (
              <div key={item.id} className="rounded-xl bg-slate-50 p-4">
                <p className="text-sm font-semibold text-slate-950">{item.label}</p>
                <p className="mt-1 text-xs text-slate-500">
      ...
      <div className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-4">
        ...
        <div className="mt-3 grid gap-3 md:grid-cols-2">
          {report.providerCallFindings.map((finding) => (
            <div key={finding.id} className="rounded-xl bg-white p-4">
              <p className="text-sm font-semibold text-slate-950">{finding.label}</p>
              <p className="mt-1 text-xs text-slate-500">
                {finding.lane} · {finding.severity} · {finding.blocksProviderCallActivation ? "blocks activation" : "review"}
              </p>
