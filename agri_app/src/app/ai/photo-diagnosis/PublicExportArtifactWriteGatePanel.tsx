...
export default function PublicExportArtifactWriteGatePanel() {
  const report = useMemo(() => buildAiPublicExportArtifactWriteGateReport(), []);
  const failedGates = report.artifactWriteGateChecks.filter((gate) => !gate.passed);
  const blockingFindings = report.artifactWriteFindings.filter((item) => item.blocksArtifactWriteActivation);
  const noGoItems = report.artifactWriteNoGoBoard.filter((item) => item.goNoGoState === "no-go");
    ...
    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.22em] text-emerald-700">
            V17.8 · AI Public Export Artifact Write Gate
          </p>
          <h2 className="mt-2 text-2xl font-bold text-slate-950">
            Public Export Artifact Write Gate & Legal Privacy Final Artifact Approval Lock
          </h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
            Gate locale dry-run per futura scrittura controllata degli artifact export pubblici,
            con approvazione legal/privacy finale, manifest boundary, checksum boundary,
            archive boundary e rollback artifact plan. Nessun artifact export viene scritto.
          </p>
        </div>

        <div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-900">
          <p className="font-semibold">Artifact write: {report.artifactWriteStatus}</p>
          <p className="mt-1">Score: {report.artifactWriteScore}/100 · severity: {report.overallSeverity}</p>
        </div>
      </div>
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-5">
        <div className="rounded-2xl bg-slate-50 p-4">
          ...
          <p className="mt-2 text-2xl font-bold text-slate-950">
            {report.readiness.publicExportArtifactWriteGateReady ? "READY" : "NO"}
          </p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
          <p className="text-xs uppercase tracking-wide text-slate-500">Artifact write</p>
          <p className="mt-2 text-2xl font-bold text-slate-950">
            {report.readiness.publicExportArtifactWriteAllowed ? "YES" : "LOCKED"}
          </p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-2">
        <div className="rounded-2xl border border-slate-200 p-4">
          ...
          <div className="mt-4 space-y-3">
            {report.artifactWriteGate.map((item) => (
          ...
          <div className="mt-4 space-y-3">
            {report.manifestWriteBoundary.map((item) => (
              <div key={item.id} className="rounded-xl bg-slate-50 p-4">
                <p className="text-sm font-semibold text-slate-950">{item.label}</p>
                <p className="mt-1 text-xs text-slate-500">
        ...
        <div className="rounded-2xl border border-slate-200 p-4">
          ...
          <div className="mt-4 space-y-3">
            {report.artifactWriteGateChecks.map((gate) => (
          ...
          <div className="mt-4 space-y-3">
            {report.artifactWriteNoGoBoard.map((item) => (
              <div key={item.id} className="rounded-xl bg-slate-50 p-4">
                <p className="text-sm font-semibold text-slate-950">{item.label}</p>
                <p className="mt-1 text-xs text-slate-500">
      ...
      <div className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-4">
        ...
        <div className="mt-3 grid gap-3 md:grid-cols-2">
          {report.artifactWriteFindings.map((finding) => (
            <div key={finding.id} className="rounded-xl bg-white p-4">
              <p className="text-sm font-semibold text-slate-950">{finding.label}</p>
              <p className="mt-1 text-xs text-slate-500">
                {finding.lane} · {finding.severity} · {finding.blocksArtifactWriteActivation ? "blocks activation" : "review"}
              </p>
