...
export default function TesterSecondTesterManualUatScenarioPackPanel() {
  const report = useMemo(() => buildTesterSecondTesterManualUatScenarioPackReport(), []);
  const blockingFindings = report.findings.filter((item) => item.blocksScenarioPack);

  return (
    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <p className="text-xs font-semibold uppercase tracking-[0.22em] text-emerald-700">
        V21.2 · Second Tester Manual UAT
      </p>
      <h2 className="mt-2 text-2xl font-bold text-slate-950">
        Second Tester Manual UAT Scenario Pack & Evidence Checklist
      </h2>
      <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
        Scenario pack read-only per secondo tester: login, navigazione, photo diagnosis, evidence bundle, mobile, lingua, logout e checklist evidenze.
      </p>
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-5">
        ...
        <div className="rounded-2xl bg-slate-50 p-4">
          ...
          <p className="mt-2 text-2xl font-bold text-slate-950">{report.scenarioScore}</p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
