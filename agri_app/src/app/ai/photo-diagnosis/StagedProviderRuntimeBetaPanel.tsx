...
export default function StagedProviderRuntimeBetaPanel() {
  const report = useMemo(() => buildAiStagedProviderRuntimeBetaReport(), []);
  const failedGates = report.runtimeBetaGates.filter((gate) => !gate.passed);
  const blockingFindings = report.runtimeBetaFindings.filter((item) => item.blocksRuntimeBetaActivation);
  ...
  return (
    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.22em] text-emerald-700">
            V16.4 · AI Staged Provider Runtime Beta
          </p>
          <h2 className="mt-2 text-2xl font-bold text-slate-950">
            Staged Provider Runtime Beta Gate & Explicit Activation Approval Lock
          </h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
            Gate locale dry-run per allowlist runtime, provider contract, budget window,
            canary scope, runtime stop e reviewer approval board. Nessuna chiamata provider viene eseguita.
          </p>
        </div>

        <div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-900">
          <p className="font-semibold">Runtime beta: {report.runtimeBetaStatus}</p>
          <p className="mt-1">Score: {report.runtimeBetaScore}/100 · severity: {report.overallSeverity}</p>
        </div>
      </div>

      <div className="mt-5 grid gap-4 md:grid-cols-5">
        <div className="rounded-2xl bg-slate-50 p-4">
          <p className="text-xs uppercase tracking-wide text-slate-500">Beta plan</p>
          <p className="mt-2 text-2xl font-bold text-slate-950">
            {report.readiness.stagedProviderRuntimeBetaReady ? "READY" : "NO"}
          </p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
          <p className="text-xs uppercase tracking-wide text-slate-500">Runtime</p>
          <p className="mt-2 text-2xl font-bold text-slate-950">
            {report.readiness.providerRuntimeBetaAllowed ? "YES" : "LOCKED"}
          </p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-2">
        <div className="rounded-2xl border border-slate-200 p-4">
          ...
          <div className="mt-4 space-y-3">
            {report.runtimeAllowlist.map((item) => (
              <div key={item.id} className="rounded-xl bg-slate-50 p-4">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <span className="rounded-full bg-white px-3 py-1 text-xs font-semibold text-emerald-700">
                    {item.allowlistScore}/100
                  </span>
                </div>
                <p className="mt-2 text-xs text-slate-600">{item.allowlistPurpose}</p>
          ...
          <div className="mt-4 space-y-3">
            {report.productionRuntimeLocks.map((item) => (
              <div key={item.id} className="rounded-xl bg-slate-50 p-4">
                <p className="text-sm font-semibold text-slate-950">{item.label}</p>
                <p className="mt-1 text-xs text-slate-500">
        ...
        <div className="rounded-2xl border border-slate-200 p-4">
          ...
          <div className="mt-4 space-y-3">
            {report.runtimeBetaGates.map((gate) => (
      ...
      <div className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-4">
        <h3 className="text-sm font-bold uppercase tracking-wide text-slate-700">
          Runtime beta findings
        </h3>

        <div className="mt-3 grid gap-3 md:grid-cols-2">
          {report.runtimeBetaFindings.map((finding) => (
            <div key={finding.id} className="rounded-xl bg-white p-4">
              <p className="text-sm font-semibold text-slate-950">{finding.label}</p>
              <p className="mt-1 text-xs text-slate-500">
                {finding.lane} · {finding.severity} · {finding.blocksRuntimeBetaActivation ? "blocks activation" : "review"}
              </p>
