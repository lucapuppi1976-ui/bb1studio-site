...
export default function IncidentHandlingWritePathGatePanel() {
  const report = useMemo(() => buildAiIncidentHandlingWritePathGateReport(), []);
  const failedGates = report.incidentWriteGateChecks.filter((gate) => !gate.passed);
  const blockingFindings = report.incidentWriteFindings.filter((item) => item.blocksIncidentWriteActivation);
  const noGoItems = report.incidentWriteNoGoBoard.filter((item) => item.goNoGoState === "no-go");
    ...
    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.22em] text-emerald-700">
            V17.3 · AI Incident Handling Write Path Gate
          </p>
          <h2 className="mt-2 text-2xl font-bold text-slate-950">
            Incident Handling Write Path Gate & Incident Governance Approval Lock
          </h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
            Gate locale dry-run per incident write path, governance approval,
            record boundary, escalation boundary, closure boundary e rollback incident write plan.
            Nessuna scrittura incidente viene eseguita.
          </p>
        </div>

        <div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-900">
          <p className="font-semibold">Incident write: {report.incidentWritePathStatus}</p>
          <p className="mt-1">Score: {report.incidentWritePathScore}/100 · severity: {report.overallSeverity}</p>
        </div>
      </div>
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-5">
        <div className="rounded-2xl bg-slate-50 p-4">
          ...
          <p className="mt-2 text-2xl font-bold text-slate-950">
            {report.readiness.incidentHandlingWritePathGateReady ? "READY" : "NO"}
          </p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
          <p className="text-xs uppercase tracking-wide text-slate-500">Write path</p>
          <p className="mt-2 text-2xl font-bold text-slate-950">
            {report.readiness.incidentHandlingWritePathAllowed ? "YES" : "LOCKED"}
          </p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-2">
        <div className="rounded-2xl border border-slate-200 p-4">
          ...
          <div className="mt-4 space-y-3">
            {report.incidentWritePathGate.map((item) => (
          ...
          <div className="mt-4 space-y-3">
            {report.incidentRecordBoundary.map((item) => (
              <div key={item.id} className="rounded-xl bg-slate-50 p-4">
                <p className="text-sm font-semibold text-slate-950">{item.label}</p>
                <p className="mt-1 text-xs text-slate-500">
        ...
        <div className="rounded-2xl border border-slate-200 p-4">
          ...
          <div className="mt-4 space-y-3">
            {report.incidentWriteGateChecks.map((gate) => (
          ...
          <div className="mt-4 space-y-3">
            {report.incidentWriteNoGoBoard.map((item) => (
              <div key={item.id} className="rounded-xl bg-slate-50 p-4">
                <p className="text-sm font-semibold text-slate-950">{item.label}</p>
                <p className="mt-1 text-xs text-slate-500">
      ...
      <div className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-4">
        ...
        <div className="mt-3 grid gap-3 md:grid-cols-2">
          {report.incidentWriteFindings.map((finding) => (
            <div key={finding.id} className="rounded-xl bg-white p-4">
              <p className="text-sm font-semibold text-slate-950">{finding.label}</p>
              <p className="mt-1 text-xs text-slate-500">
                {finding.lane} · {finding.severity} · {finding.blocksIncidentWriteActivation ? "blocks activation" : "review"}
              </p>
