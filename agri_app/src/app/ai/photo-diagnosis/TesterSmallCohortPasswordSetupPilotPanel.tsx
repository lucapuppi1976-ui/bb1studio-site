...
export default function TesterSmallCohortPasswordSetupPilotPanel() {
  const report = useMemo(() => buildTesterSmallCohortPasswordSetupPilotReport(), []);
  const blockingFindings = report.findings.filter((item) => item.blocksPasswordSetup);

  return (
    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <p className="text-xs font-semibold uppercase tracking-[0.22em] text-emerald-700">
        V21.8 · Protected Small-Cohort Password Setup
      </p>
      <h2 className="mt-2 text-2xl font-bold text-slate-950">
        Protected Small-Cohort Password Setup Pilot
      </h2>
      <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
        Pilot protetto per impostare solo passwordHash sugli account coorte già creati. Dry-run di default.
      </p>
