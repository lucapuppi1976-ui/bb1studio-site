...
export default function HumanSupervisedOperationsCockpitPanel() {
  const report = useMemo(() => buildAiHumanSupervisedOperationsCockpitReport(), []);
  const failedGates = report.cockpitGates.filter((gate) => !gate.passed);
  const blockingFindings = report.cockpitFindings.filter((item) => item.blocksOperationalActivation);
    ...
    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.22em] text-emerald-700">
            V16.3 · AI Human-Supervised Operations Cockpit
          </p>
          <h2 className="mt-2 text-2xl font-bold text-slate-950">
            Human-Supervised Agronomic Operations Cockpit & Manual Dispatch Readiness
          </h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
            Cockpit locale dry-run per operator board, manual dispatch readiness, reviewer queue,
            escalation path, safety boundary e operational evidence pack. Nessun dispatch o runtime viene attivato.
          </p>
        </div>

        <div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-900">
          <p className="font-semibold">Cockpit: {report.cockpitStatus}</p>
          <p className="mt-1">Score: {report.cockpitScore}/100 · severity: {report.overallSeverity}</p>
        </div>
      </div>

      <div className="mt-5 grid gap-4 md:grid-cols-5">
        <div className="rounded-2xl bg-slate-50 p-4">
          <p className="text-xs uppercase tracking-wide text-slate-500">Cockpit</p>
          <p className="mt-2 text-2xl font-bold text-slate-950">
            {report.readiness.humanSupervisedCockpitReady ? "READY" : "NO"}
          </p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
          <p className="text-xs uppercase tracking-wide text-slate-500">Dispatch</p>
          <p className="mt-2 text-2xl font-bold text-slate-950">
            {report.readiness.manualConversionAllowed ? "YES" : "LOCKED"}
          </p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-2">
        <div className="rounded-2xl border border-slate-200 p-4">
          ...
          <div className="mt-4 space-y-3">
            {report.operatorBoard.map((item) => (
              <div key={item.id} className="rounded-xl bg-slate-50 p-4">
                ...
                <p className="mt-2 text-xs text-slate-600">{item.boardPurpose}</p>
          ...
          <div className="mt-4 space-y-3">
            {report.safetyBoundary.map((item) => (
              <div key={item.id} className="rounded-xl bg-slate-50 p-4">
                <p className="text-sm font-semibold text-slate-950">{item.label}</p>
                <p className="mt-1 text-xs text-slate-500">
      ...
      <div className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-4">
        ...
        <div className="mt-3 grid gap-3 md:grid-cols-2">
          {report.cockpitFindings.map((finding) => (
            <div key={finding.id} className="rounded-xl bg-white p-4">
              <p className="text-sm font-semibold text-slate-950">{finding.label}</p>
              <p className="mt-1 text-xs text-slate-500">
                {finding.lane} · {finding.severity} · {finding.blocksOperationalActivation ? "blocks activation" : "review"}
              </p>
