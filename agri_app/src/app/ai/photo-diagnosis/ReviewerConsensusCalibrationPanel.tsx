...
export default function ReviewerConsensusCalibrationPanel() {
  const report = useMemo(() => buildAiReviewerConsensusCalibrationReport(), []);
  const failedGates = report.disputeResolutionGates.filter((gate) => !gate.passed);
  const blockingRisks = report.consensusRiskRegister.filter((item) => item.blocksConsensus);
  const blockedSignoffs = report.consensusSignoff.filter((item) => !item.signoffReady);
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.22em] text-emerald-700">
            V13.1 · AI Reviewer Consensus Calibration
          </p>
          <h2 className="mt-2 text-2xl font-bold text-slate-950">
            Reviewer Consensus Calibration & Dispute Resolution Engine
          </h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
            Motore locale dry-run per consenso tra reviewer, evidence weighting,
            dissent register, calibration board, dispute gates, audit trail e signoff manuale.
            Nessun provider viene chiamato e nessun output operativo viene prodotto.
          </p>
        </div>

        <div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-900">
          <p className="font-semibold">Consensus status: {report.consensusStatus}</p>
          <p className="mt-1">Score: {report.consensusScore}/100 · severity: {report.overallSeverity}</p>
        </div>
      </div>

      <div className="mt-5 grid gap-4 md:grid-cols-5">
        <div className="rounded-2xl bg-slate-50 p-4">
          <p className="text-xs uppercase tracking-wide text-slate-500">Weighting</p>
          <p className="mt-2 text-3xl font-bold text-slate-950">{report.evidenceWeighting.length}</p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
        ...
        <div className="rounded-2xl bg-slate-50 p-4">
          ...
          <p className="mt-2 text-3xl font-bold text-slate-950">{report.dissentRegister.length}</p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-2">
        <div className="rounded-2xl border border-slate-200 p-4">
          ...
          <div className="mt-4 space-y-3">
            {report.evidenceWeighting.map((item) => (
          ...
          <div className="mt-4 space-y-3">
            {report.decisionHolds.map((item) => (
              <div key={item.id} className="rounded-xl bg-slate-50 p-4">
                <p className="text-sm font-semibold text-slate-950">{item.label}</p>
                <p className="mt-1 text-xs text-slate-500">
                  {item.lane} · {item.severity} · {item.reviewer}
                </p>
                <p className="mt-2 text-xs text-slate-600">{item.holdStatement}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-slate-200 p-4">
          ...
          <div className="mt-4 space-y-3">
            {report.disputeResolutionGates.map((gate) => (
          ...
          <div className="mt-4 space-y-3">
            {report.consensusSignoff.map((item) => (
              <div key={item.id} className="rounded-xl bg-slate-50 p-4">
                <p className="text-sm font-semibold text-slate-950">{item.label}</p>
                <p className="mt-1 text-xs text-slate-500">
