...
export default function IncidentRecordPersistenceGovernancePanel() {
  const report = useMemo(() => buildAiIncidentRecordPersistenceGovernanceReport(), []);
  const blockingFindings = report.findings.filter((item) => item.blocksPersistenceActivation);
    ...
    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.22em] text-emerald-700">
            V18.1 · Incident Record Persistence Governance
          </p>
          <h2 className="mt-2 text-2xl font-bold text-slate-950">
            Incident Record Persistence Governance Monitor & Final Approval Evidence Vault
          </h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
            Monitor locale dry-run per approval evidence, persistence boundary, evidence vault,
            retention review e rollback readiness. Nessun record incidente viene persistito.
          </p>
        </div>

        <div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-900">
          <p className="font-semibold">Status: {report.persistenceGovernanceStatus}</p>
          <p className="mt-1">Score: {report.persistenceGovernanceScore}/100</p>
        </div>
      </div>
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-5">
        ...
        <div className="rounded-2xl bg-slate-50 p-4">
          ...
          <p className="mt-2 text-3xl font-bold text-slate-950">{report.approvalEvidenceBoard.length}</p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-2">
        <div className="rounded-2xl border border-slate-200 p-4">
          ...
          <div className="mt-4 space-y-3">
            {report.approvalEvidenceBoard.map((item) => (
        ...
        <div className="rounded-2xl border border-slate-200 p-4">
          ...
          <div className="mt-4 space-y-3">
            {report.persistenceBoundaryBoard.map((item) => (
              <div key={item.id} className="rounded-xl bg-slate-50 p-4">
                <p className="text-sm font-semibold text-slate-950">{item.label}</p>
                <p className="mt-1 text-xs text-slate-500">{item.lane} · {item.priority} · {item.score}/100</p>
                <p className="mt-2 text-xs text-slate-600">{item.safeOutcome}</p>
