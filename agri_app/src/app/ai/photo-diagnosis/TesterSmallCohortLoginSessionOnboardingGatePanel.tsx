...
export default function TesterSmallCohortLoginSessionOnboardingGatePanel() {
  const report = useMemo(() => buildTesterSmallCohortLoginSessionOnboardingReport(), []);
  const blockingFindings = report.findings.filter((item) => item.blocksUatProgression);

  return (
    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <p className="text-xs font-semibold uppercase tracking-[0.22em] text-emerald-700">
        V21.9 · Small-Cohort Login Session & Onboarding
      </p>
      <h2 className="mt-2 text-2xl font-bold text-slate-950">
        Small-Cohort Login Session Verification & Manual Onboarding Gate
      </h2>
      <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
        Gate read-only per confermare login, sessione, onboarding manuale e sicurezza della piccola coorte.
      </p>
