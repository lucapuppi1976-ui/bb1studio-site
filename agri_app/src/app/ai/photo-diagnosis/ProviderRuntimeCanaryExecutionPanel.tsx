...
export default function ProviderRuntimeCanaryExecutionPanel() {
  const report = useMemo(() => buildAiProviderRuntimeCanaryExecutionReport(), []);
  const failedGates = report.canaryGateChecks.filter((gate) => !gate.passed);
  const blockingFindings = report.canaryFindings.filter((item) => item.blocksCanaryActivation);
  const noGoItems = report.canaryNoGoBoard.filter((item) => item.goNoGoState === "no-go");
    ...
    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.22em] text-emerald-700">
            V16.8 · AI Provider Runtime Canary Execution
          </p>
          <h2 className="mt-2 text-2xl font-bold text-slate-950">
            Provider Runtime Canary Execution Gate & Zero-Call Execution Lock
          </h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
            Gate locale dry-run per canary execution, zero-call lock, canary criteria,
            budget envelope, reviewer approval e provider result boundary. Nessuna chiamata provider viene eseguita.
          </p>
        </div>

        <div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-900">
          <p className="font-semibold">Canary gate: {report.canaryGateStatus}</p>
          <p className="mt-1">Score: {report.canaryGateScore}/100 · severity: {report.overallSeverity}</p>
        </div>
      </div>
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-5">
        <div className="rounded-2xl bg-slate-50 p-4">
          ...
          <p className="mt-2 text-2xl font-bold text-slate-950">
            {report.readiness.providerRuntimeCanaryExecutionGateReady ? "READY" : "NO"}
          </p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
          <p className="text-xs uppercase tracking-wide text-slate-500">Canary call</p>
          <p className="mt-2 text-2xl font-bold text-slate-950">
            {report.readiness.providerCanaryCallAllowed ? "YES" : "LOCKED"}
          </p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-2">
        <div className="rounded-2xl border border-slate-200 p-4">
          ...
          <div className="mt-4 space-y-3">
            {report.canaryExecutionGate.map((item) => (
          ...
          <div className="mt-4 space-y-3">
            {report.providerZeroCallLocks.map((item) => (
              <div key={item.id} className="rounded-xl bg-slate-50 p-4">
                <p className="text-sm font-semibold text-slate-950">{item.label}</p>
                <p className="mt-1 text-xs text-slate-500">
        ...
        <div className="rounded-2xl border border-slate-200 p-4">
          ...
          <div className="mt-4 space-y-3">
            {report.canaryGateChecks.map((gate) => (
          ...
          <div className="mt-4 space-y-3">
            {report.canaryNoGoBoard.map((item) => (
              <div key={item.id} className="rounded-xl bg-slate-50 p-4">
                <p className="text-sm font-semibold text-slate-950">{item.label}</p>
                <p className="mt-1 text-xs text-slate-500">
      ...
      <div className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-4">
        ...
        <div className="mt-3 grid gap-3 md:grid-cols-2">
          {report.canaryFindings.map((finding) => (
            <div key={finding.id} className="rounded-xl bg-white p-4">
              <p className="text-sm font-semibold text-slate-950">{finding.label}</p>
              <p className="mt-1 text-xs text-slate-500">
                {finding.lane} · {finding.severity} · {finding.blocksCanaryActivation ? "blocks activation" : "review"}
              </p>
