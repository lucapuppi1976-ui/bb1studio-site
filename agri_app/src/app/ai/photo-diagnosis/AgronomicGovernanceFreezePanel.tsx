...
import { buildAiAgronomicGovernanceFreezeReport } from "@/lib/ai/aiAgronomicGovernanceFreeze";

export default function AgronomicGovernanceFreezePanel() {
  const report = useMemo(() => buildAiAgronomicGovernanceFreezeReport(), []);
  const failedGates = report.governanceGateMatrix.filter((gate) => !gate.passed);
  const blockingRisks = report.governanceRiskRegister.filter((item) => item.blocksGovernanceBoard);
  const blockedSignoffs = report.governanceSignoff.filter((item) => !item.signoffReady);
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.22em] text-emerald-700">
            V14.4 · AI Agronomic Governance Freeze
          </p>
          <h2 className="mt-2 text-2xl font-bold text-slate-950">
            Agronomic Governance Freeze & Strategy Signoff Registry
          </h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
            Board locale dry-run per governance freeze, registry di signoff, actionability trap,
            scenario closure, decision boundary ledger, non-execution certificate e signoff umano.
            Nessun provider viene chiamato e nessun output operativo viene prodotto.
          </p>
        </div>

        <div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-900">
          <p className="font-semibold">Governance status: {report.governanceStatus}</p>
          <p className="mt-1">Score: {report.governanceScore}/100 · severity: {report.overallSeverity}</p>
        </div>
      </div>

      <div className="mt-5 grid gap-4 md:grid-cols-5">
        <div className="rounded-2xl bg-slate-50 p-4">
          <p className="text-xs uppercase tracking-wide text-slate-500">Freeze</p>
          <p className="mt-2 text-3xl font-bold text-slate-950">{report.governanceFreezeBoard.length}</p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
        ...
        <div className="rounded-2xl bg-slate-50 p-4">
          <p className="text-xs uppercase tracking-wide text-slate-500">Traps</p>
          <p className="mt-2 text-3xl font-bold text-slate-950">{report.actionabilityTrapBoard.length}</p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-2">
        <div className="rounded-2xl border border-slate-200 p-4">
          ...
          <div className="mt-4 space-y-3">
            {report.governanceFreezeBoard.map((item) => (
              <div key={item.id} className="rounded-xl bg-slate-50 p-4">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <span className="rounded-full bg-white px-3 py-1 text-xs font-semibold text-emerald-700">
                    {item.freezeScore}/100
                  </span>
                </div>
                <p className="mt-2 text-xs text-slate-600">{item.freezeQuestion}</p>
        ...
        <div className="rounded-2xl border border-slate-200 p-4">
          ...
          <div className="mt-4 space-y-3">
            {report.governanceGateMatrix.map((gate) => (
          ...
          <div className="mt-4 space-y-3">
            {report.governanceSignoff.map((item) => (
              <div key={item.id} className="rounded-xl bg-slate-50 p-4">
                <p className="text-sm font-semibold text-slate-950">{item.label}</p>
                <p className="mt-1 text-xs text-slate-500">
