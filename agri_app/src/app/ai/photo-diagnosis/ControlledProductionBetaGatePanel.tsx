...
export default function ControlledProductionBetaGatePanel() {
  const report = useMemo(() => buildAiControlledProductionBetaGateReport(), []);
  const failedGates = report.launchGates.filter((gate) => !gate.passed);
  const blockingFindings = report.launchBoardFindings.filter((item) => item.blocksLaunchActivation);
    ...
    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.22em] text-emerald-700">
            V16.0 · AI Controlled Production Beta Gate
          </p>
          <h2 className="mt-2 text-2xl font-bold text-slate-950">
            Controlled Production Beta Launch Gate & Zero-Activation Cutover Plan
          </h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
            Gate locale dry-run per futura beta controllata: cutover plan, runtime locks,
            observability tower, rollback tower e go/no-go board. Zero attivazioni, zero chiamate provider,
            zero persistenza AI.
          </p>
        </div>

        <div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-900">
          <p className="font-semibold">Launch gate: {report.launchGateStatus}</p>
          <p className="mt-1">Score: {report.launchGateScore}/100 · severity: {report.overallSeverity}</p>
        </div>
      </div>
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-5">
        <div className="rounded-2xl bg-slate-50 p-4">
          ...
          <p className="mt-2 text-2xl font-bold text-slate-950">
            {report.readiness.productionBetaGateReady ? "READY" : "NO"}
          </p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
          <p className="text-xs uppercase tracking-wide text-slate-500">Beta</p>
          <p className="mt-2 text-2xl font-bold text-slate-950">
            {report.readiness.productionBetaAllowed ? "YES" : "LOCKED"}
          </p>
        </div>
        <div className="rounded-2xl bg-slate-50 p-4">
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-2">
        <div className="rounded-2xl border border-slate-200 p-4">
          ...
          <div className="mt-4 space-y-3">
            {report.launchCutoverPlan.map((item) => (
              <div key={item.id} className="rounded-xl bg-slate-50 p-4">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <span className="rounded-full bg-white px-3 py-1 text-xs font-semibold text-emerald-700">
                    {item.cutoverScore}/100
                  </span>
                </div>
                <p className="mt-2 text-xs text-slate-600">{item.cutoverPurpose}</p>
          ...
          <div className="mt-4 space-y-3">
            {[
              ...report.providerRuntimeLocks,
              ...report.storageRuntimeLocks,
              ...report.reviewRuntimeLocks,
              ...report.conversionRuntimeLocks,
            ].map((item) => (
              <div key={item.id} className="rounded-xl bg-slate-50 p-4">
                <p className="text-sm font-semibold text-slate-950">{item.label}</p>
                <p className="mt-1 text-xs text-slate-500">
      ...
      <div className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-4">
        ...
        <div className="mt-3 grid gap-3 md:grid-cols-2">
          {report.launchBoardFindings.map((finding) => (
            <div key={finding.id} className="rounded-xl bg-white p-4">
              <p className="text-sm font-semibold text-slate-950">{finding.label}</p>
              <p className="mt-1 text-xs text-slate-500">
                {finding.lane} · {finding.severity} · {finding.blocksLaunchActivation ? "blocks activation" : "review"}
              </p>
