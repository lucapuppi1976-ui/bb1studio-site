...
export async function GET(request: NextRequest) {
  ...
  return NextResponse.json({
    ok: true,
    endpoint: "/api/ops/ai-farm-command-board-dry-run",
    ...
    report,
    textReport: formatFarmCommandBoardReport(report),
  });
}
...
export async function POST(request: NextRequest) {
  ...
  let input: FarmCommandBoardInput = defaultFarmCommandBoardInput;

  try {
    const payload = await request.json();
    input = payload?.input || input;
  } catch {
    input = defaultFarmCommandBoardInput;
  }

  const report = createFarmCommandBoardReport(input);
