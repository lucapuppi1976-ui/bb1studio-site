...
import {
  ...
  type TesterAccountAccessVerificationInput,
} from "@/lib/ops/testerAccountAccessVerification";

export const dynamic = "force-dynamic";

type AccessVerificationBody = {
  email?: string;
};
...
async function buildAccessReport(emailInput: string) {
  ...
  const schemaHasUserModel = modelExists("User");
  const schemaHasEmailField = userFields.has("email");
  const schemaHasRoleField = userFields.has("role") || userFields.has("userRole") || userFields.has("roles");
  const schemaHasLanguageField =
    userFields.has("preferredLanguage") || userFields.has("language") || userFields.has("locale");

  let user: Record<string, unknown> | null = null;

  if (isValidEmail(email) && schemaHasUserModel && schemaHasEmailField) {
  ...
  const roleValue =
    String(user?.role ?? user?.userRole ?? "").trim();

  const input: TesterAccountAccessVerificationInput = {
    email,
    ...
    onboardingRouteReady: true,
    protectedRouteReady: true,
  };

  const report = buildTesterAccountAccessVerificationReport(input);
...
export async function GET(request: NextRequest) {
    ...
    return NextResponse.json(
      {
        ...
        endpoint: "/api/ops/tester-account-access-verification-dry-run",
      },
      { status: 403 },
    );
  }
  ...
  const result = await buildAccessReport(email);
...
export async function POST(request: NextRequest) {
  ...
  const body = (await request.json().catch(() => ({}))) as AccessVerificationBody;
  const result = await buildAccessReport(body.email ?? "");
