...
import {
  ...
  type TesterExtendedSingleTesterUatInput,
} from "@/lib/ops/testerExtendedSingleTesterUatCoverage";

export const dynamic = "force-dynamic";

type ExtendedSingleTesterUatBody = Omit<
  TesterExtendedSingleTesterUatInput,
...
async function buildExtendedSingleTesterUatReport(body: ExtendedSingleTesterUatBody) {
  ...
  const input: TesterExtendedSingleTesterUatInput = {
    ...body,
    email,
  ...
  const report = buildTesterExtendedSingleTesterUatCoverageReport(input);
    ...
    return NextResponse.json(
      {
        ...
        endpoint: "/api/ops/tester-extended-single-tester-uat-coverage-dry-run",
      },
      { status: 403 },
    );
  }

  const result = await buildExtendedSingleTesterUatReport({
...
export async function POST(request: NextRequest) {
  ...
  const body = (await request.json().catch(() => ({}))) as ExtendedSingleTesterUatBody;
  const result = await buildExtendedSingleTesterUatReport(body);
