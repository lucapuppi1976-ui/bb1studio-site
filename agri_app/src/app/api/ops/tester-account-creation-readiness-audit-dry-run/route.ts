...
import {
  ...
  type TesterAccountCreationReadinessInput,
} from "@/lib/ops/testerAccountCreationReadinessAudit";
    ...
    return NextResponse.json(
      {
        ...
        endpoint: "/api/ops/tester-account-creation-readiness-audit-dry-run",
      },
      { status: 403 },
    );
  }
...
export async function POST(request: NextRequest) {
  ...
  const body = (await request.json().catch(() => ({}))) as TesterAccountCreationReadinessInput;
  const report = buildTesterAccountCreationReadinessAuditReport(body);
