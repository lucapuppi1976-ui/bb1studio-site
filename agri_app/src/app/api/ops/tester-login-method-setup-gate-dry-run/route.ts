...
import {
  ...
  type TesterLoginMethodSetupInput,
} from "@/lib/ops/testerLoginMethodSetupGate";

export const dynamic = "force-dynamic";

type LoginMethodSetupBody = {
  email?: string;
};
...
async function buildLoginMethodReport(emailInput: string) {
  ...
  const input: TesterLoginMethodSetupInput = {
    email,
  ...
  const report = buildTesterLoginMethodSetupReport(input);
...
export async function GET(request: NextRequest) {
    ...
    return NextResponse.json(
      {
        ...
        endpoint: "/api/ops/tester-login-method-setup-gate-dry-run",
      },
      { status: 403 },
    );
  }
  ...
  const result = await buildLoginMethodReport(email);
...
export async function POST(request: NextRequest) {
  ...
  const body = (await request.json().catch(() => ({}))) as LoginMethodSetupBody;
  const result = await buildLoginMethodReport(body.email ?? "");
