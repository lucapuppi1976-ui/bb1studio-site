...
import {
  ...
  type TesterManualUatExecutionReportInput,
} from "@/lib/ops/testerManualUatExecutionReport";

export const dynamic = "force-dynamic";

type ManualUatExecutionBody = Omit<
  TesterManualUatExecutionReportInput,
...
async function buildExecutionReport(body: ManualUatExecutionBody) {
  ...
  const input: TesterManualUatExecutionReportInput = {
    ...body,
    email,
  ...
  const report = buildTesterManualUatExecutionReport(input);
    ...
    return NextResponse.json(
      {
        ...
        endpoint: "/api/ops/tester-manual-uat-execution-report-dry-run",
      },
      { status: 403 },
    );
  }
...
export async function POST(request: NextRequest) {
  ...
  const body = (await request.json().catch(() => ({}))) as ManualUatExecutionBody;
