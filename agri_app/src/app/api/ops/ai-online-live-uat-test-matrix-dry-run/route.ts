...
import {
  ...
  type LiveUatInput,
} from "@/lib/ai/aiOnlineLiveUatTestMatrix";
    ...
    return NextResponse.json(
      {
        ...
        endpoint: "/api/ops/ai-online-live-uat-test-matrix-dry-run",
      },
      { status: 403 },
    );
  }
...
export async function POST(request: NextRequest) {
  ...
  const body = (await request.json().catch(() => ({}))) as LiveUatInput;
  const report = buildAiOnlineLiveUatTestMatrixReport(body);
