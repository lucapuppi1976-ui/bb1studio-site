...
import {
  ...
  type LiveUatLaunchInput,
} from "@/lib/ai/aiLiveUatLaunchGate";
    ...
    return NextResponse.json(
      {
        ...
        endpoint: "/api/ops/ai-live-uat-launch-gate-dry-run",
      },
      { status: 403 },
    );
  }
...
export async function POST(request: NextRequest) {
  ...
  const body = (await request.json().catch(() => ({}))) as LiveUatLaunchInput;
  const report = buildAiLiveUatLaunchGateReport(body);
