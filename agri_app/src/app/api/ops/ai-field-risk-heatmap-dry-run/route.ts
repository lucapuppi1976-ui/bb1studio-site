...
export async function GET(request: NextRequest) {
  ...
  return NextResponse.json({
    ok: true,
    endpoint: "/api/ops/ai-field-risk-heatmap-dry-run",
    ...
    report,
    textReport: formatFieldRiskHeatmapReport(report),
  });
}
...
export async function POST(request: NextRequest) {
  ...
  let input: FieldRiskHeatmapInput = defaultFieldRiskHeatmapInput;

  try {
    const payload = await request.json();
    input = payload?.input || input;
  } catch {
    input = defaultFieldRiskHeatmapInput;
  }

  const report = createFieldRiskHeatmapReport(input);
