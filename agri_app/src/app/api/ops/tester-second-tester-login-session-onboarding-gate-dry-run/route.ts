...
import {
  ...
  type TesterSecondTesterLoginSessionOnboardingInput,
} from "@/lib/ops/testerSecondTesterLoginSessionOnboardingGate";

export const dynamic = "force-dynamic";

type SecondTesterLoginSessionOnboardingBody = Omit<
  TesterSecondTesterLoginSessionOnboardingInput,
  | "firstTesterDetected"
...
async function buildLoginSessionOnboardingReport(
  body: SecondTesterLoginSessionOnboardingBody,
) {
  ...
  const input: TesterSecondTesterLoginSessionOnboardingInput = {
    ...body,
    firstTesterEmail,
    secondTesterEmail,
  ...
  const report = buildTesterSecondTesterLoginSessionOnboardingReport(input);
    ...
    return NextResponse.json(
      {
        ...
        endpoint: "/api/ops/tester-second-tester-login-session-onboarding-gate-dry-run",
      },
      { status: 403 },
    );
  }

  const result = await buildLoginSessionOnboardingReport({
...
export async function POST(request: NextRequest) {
  ...
  const body = (await request.json().catch(() => ({}))) as SecondTesterLoginSessionOnboardingBody;
  const result = await buildLoginSessionOnboardingReport(body);
