...
} from "@/lib/ai/aiUatFeedbackBugEvidenceSession";
    ...
    return NextResponse.json(
      {
        ...
        endpoint: "/api/ops/ai-uat-feedback-bug-evidence-session-dry-run",
      },
      { status: 403 },
    );
  }
...
export async function POST(request: NextRequest) {
  ...
  const body = (await request.json().catch(() => ({}))) as UatSessionInput;
  const report = buildAiUatFeedbackBugEvidenceSessionReport(body);
