...
import {
  ...
  type UxHardeningInput,
} from "@/lib/ai/aiUxNavigationHardening";
    ...
    return NextResponse.json(
      {
        ...
        endpoint: "/api/ops/ai-ux-navigation-hardening-dry-run",
      },
      { status: 403 },
    );
  }
...
export async function POST(request: NextRequest) {
  ...
  const body = (await request.json().catch(() => ({}))) as UxHardeningInput;
  const report = buildAiUxNavigationHardeningReport(body);
