...
import {
  ...
  type IncidentPersistenceInput,
} from "@/lib/ai/aiIncidentRecordPersistenceGovernance";
    ...
    return NextResponse.json(
      {
        ...
        endpoint: "/api/ops/ai-incident-record-persistence-governance-dry-run",
      },
      { status: 403 },
    );
  }
...
export async function POST(request: NextRequest) {
  ...
  const body = (await request.json().catch(() => ({}))) as IncidentPersistenceInput;
  const report = buildAiIncidentRecordPersistenceGovernanceReport(body);
