...
import {
  ...
  type TesterProvisioningInput,
} from "@/lib/ai/aiTesterProvisioningAdapter";
    ...
    return NextResponse.json(
      {
        ...
        endpoint: "/api/ops/ai-tester-provisioning-adapter-dry-run",
      },
      { status: 403 },
    );
  }
...
export async function POST(request: NextRequest) {
  ...
  const body = (await request.json().catch(() => ({}))) as TesterProvisioningInput;
  const report = buildAiTesterProvisioningAdapterReport(body);
