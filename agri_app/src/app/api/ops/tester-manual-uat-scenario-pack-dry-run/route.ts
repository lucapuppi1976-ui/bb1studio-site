...
import {
  ...
  type TesterManualUatScenarioPackInput,
} from "@/lib/ops/testerManualUatScenarioPack";

export const dynamic = "force-dynamic";

type ManualUatScenarioPackBody = {
  email?: string;
...
async function buildManualScenarioPack(body: ManualUatScenarioPackBody) {
  ...
  const input: TesterManualUatScenarioPackInput = {
    email,
    ...
    onboardingGateReady: body.onboardingGateReady ?? false,
    loginScenarioObserved: body.loginScenarioObserved ?? false,
    navigationScenarioObserved: body.navigationScenarioObserved ?? false,
    photoDiagnosisScenarioObserved: body.photoDiagnosisScenarioObserved ?? false,
    operationsReadOnlyScenarioObserved: body.operationsReadOnlyScenarioObserved ?? false,
    mobileUsabilityObserved: body.mobileUsabilityObserved ?? false,
    localeFallbackObserved: body.localeFallbackObserved ?? false,
    logoutScenarioObserved: body.logoutScenarioObserved ?? false,
    evidenceScreenshotsCaptured: body.evidenceScreenshotsCaptured ?? false,
    evidenceNotesCaptured: body.evidenceNotesCaptured ?? false,
    ...
    noAccountWriteConfirmed: body.noAccountWriteConfirmed ?? false,
    noSchemaMigrationConfirmed: body.noSchemaMigrationConfirmed ?? false,
    openCriticalIssueCount: body.openCriticalIssueCount ?? 0,
    openMajorIssueCount: body.openMajorIssueCount ?? 0,
  };

  const report = buildTesterManualUatScenarioPackReport(input);
    ...
    return NextResponse.json(
      {
        ...
        endpoint: "/api/ops/tester-manual-uat-scenario-pack-dry-run",
      },
      { status: 403 },
    );
  }

  const result = await buildManualScenarioPack({
...
export async function POST(request: NextRequest) {
  ...
  const body = (await request.json().catch(() => ({}))) as ManualUatScenarioPackBody;
  const result = await buildManualScenarioPack(body);
