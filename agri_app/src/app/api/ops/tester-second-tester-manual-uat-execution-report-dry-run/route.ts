...
import {
  ...
  type TesterSecondTesterManualUatExecutionReportInput,
} from "@/lib/ops/testerSecondTesterManualUatExecutionReport";

export const dynamic = "force-dynamic";

type SecondTesterManualUatExecutionReportBody = Omit<
  TesterSecondTesterManualUatExecutionReportInput,
  | "firstTesterDetected"
...
async function buildExecutionReport(body: SecondTesterManualUatExecutionReportBody) {
  ...
  const input: TesterSecondTesterManualUatExecutionReportInput = {
    ...body,
    firstTesterEmail,
    secondTesterEmail,
  ...
  const report = buildTesterSecondTesterManualUatExecutionReport(input);
    ...
    return NextResponse.json(
      {
        ...
        endpoint: "/api/ops/tester-second-tester-manual-uat-execution-report-dry-run",
      },
      { status: 403 },
    );
  }

  const result = await buildExecutionReport({
...
export async function POST(request: NextRequest) {
  ...
  const body = (await request.json().catch(() => ({}))) as SecondTesterManualUatExecutionReportBody;
  const result = await buildExecutionReport(body);
