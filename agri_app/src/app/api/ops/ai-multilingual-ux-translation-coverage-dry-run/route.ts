...
import {
  ...
  type TranslationCoverageInput,
} from "@/lib/ai/aiMultilingualUxTranslationCoverage";
    ...
    return NextResponse.json(
      {
        ...
        endpoint: "/api/ops/ai-multilingual-ux-translation-coverage-dry-run",
      },
      { status: 403 },
    );
  }
...
export async function POST(request: NextRequest) {
  ...
  const body = (await request.json().catch(() => ({}))) as TranslationCoverageInput;
  const report = buildAiMultilingualUxTranslationCoverageReport(body);
