...
export async function GET(request: NextRequest) {
  ...
  return NextResponse.json({
    ok: true,
    endpoint: "/api/ops/ai-field-scouting-plan-dry-run",
    ...
    plan,
    textPlan: formatFieldScoutingPlan(plan),
  });
}
...
export async function POST(request: NextRequest) {
  ...
  let input: FieldScoutingInput = defaultFieldScoutingInput;

  try {
    const payload = await request.json();
    input = payload?.input || input;
  } catch {
    input = defaultFieldScoutingInput;
  }

  const plan = createFieldScoutingPlan(input);
