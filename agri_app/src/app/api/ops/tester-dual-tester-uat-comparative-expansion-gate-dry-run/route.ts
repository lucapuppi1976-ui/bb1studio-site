...
import {
  ...
  type TesterDualTesterUatComparativeExpansionInput,
} from "@/lib/ops/testerDualTesterUatComparativeExpansionGate";

export const dynamic = "force-dynamic";

type DualTesterUatComparativeExpansionBody = Omit<
  TesterDualTesterUatComparativeExpansionInput,
  | "firstTesterDetected"
...
async function buildComparativeReport(body: DualTesterUatComparativeExpansionBody) {
  ...
  const input: TesterDualTesterUatComparativeExpansionInput = {
    ...body,
    firstTesterEmail,
    secondTesterEmail,
  ...
  const report = buildTesterDualTesterUatComparativeExpansionReport(input);
    ...
    return NextResponse.json(
      {
        ...
        endpoint: "/api/ops/tester-dual-tester-uat-comparative-expansion-gate-dry-run",
      },
      { status: 403 },
    );
  }

  const result = await buildComparativeReport({
...
export async function POST(request: NextRequest) {
  ...
  const body = (await request.json().catch(() => ({}))) as DualTesterUatComparativeExpansionBody;
  const result = await buildComparativeReport(body);
